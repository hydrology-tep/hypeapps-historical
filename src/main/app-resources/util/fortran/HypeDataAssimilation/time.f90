...
!>Module for time routines. Contains procedures relating to time calculations.
MODULE TIMEROUTINES

  !Copyright 2011-2013,2015-2016 SMHI
  !
...
CONTAINS
  ...
  INTEGER FUNCTION day_of_month(year,month)
    ...
    CASE DEFAULT
      !ERROR
      WRITE(6,*) 'ERROR in code. Trying to calculate days of month', month
      STOP 1
    END SELECT
  ...
  !!TODO: Why real function and not integer?
  !--------------------------------------------------------------------
  REAL FUNCTION numdays_of_year(date)

    !Argument decalrations
  ...
  SUBROUTINE calculate_time_for_model(idt,date)
    ...
    prevdoy = NINT(numdays_of_year(yearago))
    currentdate = date
  ...
  INTEGER FUNCTION set_timestep_variables(tstep,stepunit,fdate)
    ...
    USE WORLDVAR, ONLY : steplen, &  !OUT
                         idtlag      !OUT
    USE MODVAR, ONLY : seconds_per_timestep,timesteps_per_day !OUT
    ...
    ! Calculate step length in datetype
    ...
    set_timestep_variables = n_Result
