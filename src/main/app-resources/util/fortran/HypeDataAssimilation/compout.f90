...
!with HYPE. If not, see <http://www.gnu.org/licenses/>.
  ...
  SUBROUTINE compute_period_timesteplength_and_end_status(date,pcode,nper,pend,idt,ndt)
    ...
    INTEGER, INTENT(OUT) :: nper         !<Number of timesteps in current period
  ...
  SUBROUTINE compute_basinoutput(cd,io,iad,dim,x,pend,idt,ndt,iens)
    ...
    DO ivar = 1, output(io)%nvar
      ...
      CALL check_and_acc_one_data(simple,output(io)%variable(ivar)%vartype,y,yhelp,missing_value,output(io)%accdata%value(iad,ivar,iens),output(io)%accdata%help(iad,ivar,iens),output(io)%accdata%nok(iad,ivar,iens))
       
      !> \li If end of period: calculate output average
      IF(pend)THEN
        CALL check_and_outmean_one_data(output(io)%variable(ivar)%vartype,output(io)%period,nper,missing_value,output(io)%accdata%value(iad,ivar,iens),output(io)%accdata%help(iad,ivar,iens),x(ivar))
    ...
    DO ivar = 1,output(io)%nvar
      DO isb = 1,nsub
       !>\li Accumulate timestep data      
        ...
        CALL check_and_acc_one_data(simple,output(io)%variable(ivar)%vartype,y,yhelp,missing_value,output(io)%accdata%value(isb,ivar,tmap),output(io)%accdata%help(isb,ivar,tmap),output(io)%accdata%nok(isb,ivar,tmap))
       !> \li If end of period: calculate output average
        IF(pend)THEN
          CALL check_and_outmean_one_data(output(io)%variable(ivar)%vartype,output(io)%period,nper,missing_value,output(io)%accdata%value(isb,ivar,tmap),output(io)%accdata%help(isb,ivar,tmap),x)
  ...
  SUBROUTINE compute_timeoutput(cd,io,ivar,dim,x,pend,idt,ndt,iens)
    ...
    DO isb = 1, dim
      ...
      CALL check_and_acc_one_data(simple,output(io)%variable(ivar)%vartype,y,yhelp,missing_value,output(io)%accdata%value(isb,ivar,iens),output(io)%accdata%help(isb,ivar,iens),output(io)%accdata%nok(isb,ivar,iens))
      !> \li If end of period: calculate output average
      IF(pend)THEN
        CALL check_and_outmean_one_data(output(io)%variable(ivar)%vartype,output(io)%period,nper,missing_value,output(io)%accdata%value(isb,ivar,iens),output(io)%accdata%help(isb,ivar,iens),x(isb))
  ...
  SUBROUTINE calculate_region_average(ireg,ivar,average)
    ...
    IF(outregion(ireg)%obsfunc==0)THEN
      average=0.
      DO isb=1,outregion(ireg)%nsubbasin !loop over subbasins in region
        IF(outvar(outregion(ireg)%subindex(isb),ivar)==missing_value)THEN   !Do not calculate regional average if any of the subbasin values are missing.
          average = missing_value
          RETURN
        ENDIF
        !add subbasin outvar value multiplied by it's weight
  ...
  SUBROUTINE calculate_region_obsfunc(ireg)
    ...
    USE MODVAR, ONLY : outvar, missing_value,noutvar,seconds_per_timestep
    ...
    CASE(1) !obsfunc==1, reservoir inflow = runoff(local_catchment) + [prec - evap](reservoir)
      ...
      DO iy=1,noutvar
        IF(outvarinfo(iy)%areaagg==2 .AND. outvarinfo(iy)%idindex==o_clrf) irgclrf = iy
        IF(outvarinfo(iy)%areaagg==2 .AND. outvarinfo(iy)%idindex==o_clrp) irgclrp = iy
        IF(outvarinfo(iy)%areaagg==2 .AND. outvarinfo(iy)%idindex==o_clre) irgclre = iy
        IF(outvarinfo(iy)%areaagg==2 .AND. outvarinfo(iy)%idindex==o_qcin) irgqcin = iy
      ENDDO
      
      !2. apply the regional observation function
      IF(irgclrf>0 .AND. irgclrf>0 .AND. irgclrf>0 .AND. irgqcin>0)THEN
  ...
  !SUBROUTINE calculate_upstream_average(varindex,areatype,avvalue)
  !
  !  USE MODVAR, ONLY : nsub,outvar,upstreamweight
  !
  !  INTEGER, INTENT(IN) :: varindex     !<outvar index of variable to be averaged
  !  INTEGER, INTENT(IN) :: areatype     !<basinarea or landarea variable
  !  REAL, INTENT(OUT) :: avvalue(nsub)  !<upstream average
  ...
  !  DO isb=1,nsub
  !    DO iup=1,nsub
  !      avvalue(isb) = avvalue(isb) + upstreamweight(isb,iup,areatype)*outvar(iup,varindex)
  !    ENDDO
  !  ENDDO
  !  
  !END SUBROUTINE calculate_upstream_average
  ...
  SUBROUTINE prepare_to_compute_crit(d,idt,ndt)
    ...
    DO i = 1,nacrit
      ...
      ENDIF
!      r(1:nsub) = outvar(1:nsub,outvarindex(acccalvar(i)%rec))
!      c(1:nsub) = outvar(1:nsub,outvarindex(acccalvar(i)%comp))
!      r(1:nsub) = outvar(1:nsub,acccalvar(i)%recoutvar)
!      c(1:nsub) = outvar(1:nsub,acccalvar(i)%compoutvar)
!      IF(noreg>0)THEN
!        r((nsub+1):dim) = missing_value
!        c((nsub+1):dim) = missing_value
!        DO ir=1,noreg
!          !IF(xoregindex(acccalvar(i)%rec,ir)>0)THEN
!          !  y=xoregobsi(xoregindex(acccalvar(i)%rec,ir))
!          !ELSE
!            y=outvar(ir,acccalvar(i)%recoutvar)
!!            CALL calculate_region_average(ir,outvarindex(acccalvar(i)%rec),y)
!          !ENDIF
!          r(nsub+ir) = y
!        ENDDO
!        DO ir = 1,noreg
!          y=outvar(ir,acccalvar(i)%compoutvar)
!!          CALL calculate_region_average(ir,outvarindex(acccalvar(i)%comp),y)
!          c(nsub+ir) = y
!        ENDDO
!      ENDIF  
       
      !>Accumulate data for period
      CALL accumulate_periodvalues(missing_value,r(:),c(:),rs(1:dim,i),&
            cs(1:dim,i),ts(1:dim,i),dim)
      !>If end of period: 
      IF(pend)THEN
...
!        basincrit(:,11,iac) = locqc(:)-locqr(:)
      ...
      IF(calcsimass)THEN
        ...
        CALL calculate_subbasin_mean_var(i,iac,sr2limityear,nsubCrit,locqr,locqc)
    ...
    DO i = 1, ncrit
      IF(.NOT.calvar(i)%cond)THEN
        ...
        IF(str=='MRA')THEN                      !RA, mean of all subbasins
          CALL calculate_all_subbasins_ra(i,iac,nsubCrit,locra)
  ...
  INTEGER FUNCTION find_variable_index_type(str,outindex,timeagg,areaagg)
    ...
    IF(varstr(1:2)=='up')THEN
      areaagg = 1
      find_variable_index_type = find_variable_index(varstr(3:6),outindex,timeagg)
