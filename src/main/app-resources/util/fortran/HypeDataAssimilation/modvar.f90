...
!> \{
  ...
  INTEGER, PARAMETER :: m_mpar   = 8        !<Code för monthly dependent parameters
...
!> \brief Type for holding data about subbasin characteristics
  TYPE BASINTYPE
    ...
    REAL    :: lakedepth       !<olake water depth at threshold (m)
    ...
    REAL    :: mrwl0 = 0.      !<main river reference elevation (mrwl0+water depth = water surface elevation [m.a.s.l])
  END TYPE BASINTYPE
...
!> \{
  ...
  INTEGER :: i_t1     !<index of state variable, conservative substance 1
...
!  REAL, ALLOCATABLE :: upstreamweight(:,:,:)  !<subbasins weight to calculate upstream average for basinarea (1) or landarea (2) variable
...
!> \{
  ...
  !TODO: Should be in classdata or classbasin, or?
  REAL    soillayerdepth(maxsoillayers)           !<Lower border of soil layers (used by NHMG2) TODO: remove
...
!> \{
  ...
  INTEGER          :: noutvar      !<number of used output variables (region output)
...
!> \}
  ...
  SUBROUTINE allocate_modelparameters(ns)
    ...
    !Local variables
    INTEGER ngenpar,nsoilpar,nlandpar,nbasinpar,nregpar,nlregpar,nmonthpar,nwqregpar
    INTEGER i
  ...
  !--------------------------------------------------------------
  SUBROUTINE allocate_modeloptions()
  ...
  END SUBROUTINE allocate_modeloptions
