...
!        IF(k==i_t2) cevap(k) = 1000. * latentheat_tempfunction(riverstate%conc(k,pooltype,i)) / cwater ! 1000 * [MJ/kg] / [KJ/kg/C] = C
  ...
  SUBROUTINE point_abstraction_from_outlet_lake(i,pooltype,qunitfactor,lakestate,removedflow)
    ...
    IF(absvol<lakevol)THEN
      IF(ALLOCATED(lakestate%slowwater))THEN
        !TODO: this could be calculated smarter
  ...
  SUBROUTINE get_current_lake_outflow_parameters(i,itype,lakeareain,olakewst,   &
    ...
    REAL out2fillDamThreshold   !Percentage of reservoir capacity bellow which economy regime starts (from "par.txt")
    ...
    REAL out2qprod1, out2qprod2, out2qamp, out2qpha  !parameters of outlet 2
    REAL out2regrate, out2regexp, out2deltaw0, out2w0rel  !-"-
    INTEGER out2dayno1, out2dayno2  !-"-
    ...
    IF(itype==2)THEN
      IF(ALLOCATED(lakeindex))THEN
        IF(lakeindex(i)>0)THEN
          ...
          fillDamThreshold = lake(lakeindex(i))%limprod !fractional level when production is starting to be limited
        ENDIF
      ENDIF
      IF(ALLOCATED(lakeout2index))THEN  !Second outlet of lake/dam
        IF(lakeout2index(i)>0)THEN
          have2outlets = .TRUE.
          out2qprod1 = lake(lakeout2index(i))%qprod1          !Production discharge for period 1
          out2qprod2 = lake(lakeout2index(i))%qprod2          !                     for period 2
          out2maxProd = lake(lakeout2index(i))%mqprod         !Maximum production flow
          out2minflow = lake(lakeout2index(i))%minflow        !Minimum flow
          out2dayno1 = lake(lakeout2index(i))%datum1          !Starting day nr. for production period 1
          out2dayno2 = lake(lakeout2index(i))%datum2          !                                period 2
          out2qamp = lake(lakeout2index(i))%qamp              !Amplitude of sin-adjustment of qprod
          out2qpha = lake(lakeout2index(i))%qpha              !Phase of sin-adjustment of qprod
          out2regrate = lake(lakeout2index(i))%rate           !Rating curve parameter
          out2regexp = lake(lakeout2index(i))%exp             !Rating curve parameter
          ...
          out2fillDamThreshold = lake(lakeout2index(i))%limprod !fractional level when production is starting to be limited
        ENDIF
      ENDIF
      IF(ALLOCATED(lakebasinindex))THEN
        IF(lakebasinindex(i)>0)THEN  
          IF(lakebasin(lakebasinindex(i))%last)THEN          !Recalculate water stage for last lake basin
            ...
            deltaw0 = lake(lakebasin(lakebasinindex(i))%ilk)%deltaw0        !difference in lake threshold/"dämningsgräns" period 2
            wmin = lake(lakebasin(lakebasinindex(i))%ilk)%wmin              !lake threshold/"sänkningsgräns"
            fillDamThreshold = lake(lakebasin(lakebasinindex(i))%ilk)%limprod !fractional level when production is starting to be limited
          ELSE
      ...
      IF(ALLOCATED(damindex))THEN
        IF(damindex(i)>0)THEN
          ...
          fillDamThreshold = dam(damindex(i))%limprod !fractional level when production is starting to be limited
        ENDIF
      ENDIF
      IF(fillDamThreshold<0.) fillDamThreshold=genpar(m_limprod) !in case missing_value in LakeData, use general value
      IF(out2fillDamThreshold<0.) out2fillDamThreshold=genpar(m_limprod)
    ENDIF   !itype=2
...
!                ENDIF
      ...
      IF(have2outlets)THEN    !Calculate the second outlet
        ...
        IF(out2wmin.NE.missing_value)THEN
          qprodToday = out2qprod1                                               !Production rate 1 applies in general
          IF(out2dayno1*out2dayno2 > 0) THEN                                       !If both dates for different production regimes are non-zero, ...
            IF(dayno < out2dayno1 .OR. dayno >= out2dayno2)  qprodToday = out2qprod2   !... and today is not within the periode of production rate 1, then production rate 2 applies
          ENDIF
          IF(out2fillDamThreshold>0)THEN
            fracLevel = (wlmr - out2wmin)/(0. - out2wmin)                         !Current reservoir height, in fraction of height difference between min and top (can be negative if dam is below minimum level)
            IF(fracLevel > 0. .AND. fracLevel < out2fillDamThreshold) THEN
              qprodToday = fracLevel/out2fillDamThreshold * qprodToday        !Economy regime, if reservoir fractional filling is lower than threshold
            ENDIF
          ENDIF
          out2damProd = qprodToday*(1. + out2qamp * sin(2.*pi*(dayno+out2qpha)/365.))   !Seasonal variation according to sin-function
          IF(out2minflow) out2minProd = out2damProd     !Set minimum flow to current production flow
        ENDIF
 
        !Ordinary outflow threshold
        out2w0Today = 0. + out2w0rel

        !Current threshold for dam with production by rating curves
        IF(out2deltaw0/=0.)THEN
          IF(out2dayno1*out2dayno2 > 0) THEN                             
            IF (dayno < out2dayno1 .OR. dayno >= out2dayno2)  out2w0Today = out2w0Today + out2deltaw0   !threshold for period 2 applies
          ENDIF
        ENDIF
 
        IF(wlmr>out2w0Today)THEN
          IF(out2regrate>0.)THEN                         !Specific rating curve for lake or dam spill
            out2ratck = out2regrate
            out2ratcexp = out2regexp
          ELSEIF(out2wmin.NE.missing_value)THEN         !Dam without rating curve for spill
          ELSE                                      !General rating curve for lake
            out2ratck = rating1
            out2ratcexp = rating2
          ENDIF
  ...
  SUBROUTINE calculate_ilake_outflow(i,subid,ns,dividedlake,qin,lakearea,qunitfactor, &
    ...
    ratinge = genpar(m_grat2)
    w0Today = genpar(m_gldepi)
    ...
    CALL remove_outflow_from_lake(i,itype,ns,outflowmm,subid,1.,1.,lakewstmm,coutflow,lakestate)
    !ldepth and epidepth are not used in remove_outflow_from_lake for ilakes, set to 1.
  ...
  SUBROUTINE calculate_outflow_from_lake(i,itype,qin,lakeareain,lakewstmm,   &
    ...
    IF(itype==2)THEN
      ...
      !Outflow determination for one outlet lake (no check here)
      wlmr0 = wlmr -w0Today
      IF(upstreamlakebasin)THEN     !Upstream lake basin; all water above threshold
  ...
  SUBROUTINE check_outflow_from_lake(i,itype,lakeareain,lakewstmm, &
    ...
    CALL get_current_lake_outflow_parameters(i,itype,lakeareain,wlmr,   &
                 ...
                 wmin,damProd,maxProd,minflow,out2ratingc,out2ratinge,out2w0Today,out2wmin, &
                 out2damProd,out2maxProd,out2minflow)
    IF(have2outlets) RETURN   !Checked when calculated
    ...
    ELSE
      ...
      ELSE
        ...
        IF(outflowmm>0)THEN
          ...
          IF(q1>0.)THEN
            tempconc(:) = lakestate%conc(:,itype,i)
          ...
          ENDIF
          IF(q2>0)THEN 
            tempconc(:) = lakestate%concslow(:,itype,i)
            IF(usestrattemp) tempconc(i_t2) = t2conc
  ...
  SUBROUTINE recalculate_branched_flow(cmethod,totflow,maxQprod,maxQprod2,minflow1,minflow2,simflow1,simflow2)
    ...
    !>\i Hydropower plant with production flow and spill in different branches: check for maximum/current production flow
    ...
    !>\i Minimum flow together with production flow and spill in different branches
    ...
    ELSEIF(cmethod==7 .OR. cmethod==9)THEN
      ...
      ENDIF
    !>\i Other outflow division: keep relation between flow in the different branches
  ...
  !----------------------------------------------------------
   SUBROUTINE T2_processes_in_river(i,itype,temp,swrad,riversurft,riverarea,frozenstate,riverstate,freezeupday,freezeuparea)
    ...
    TYPE(riverstatetype),INTENT(INOUT) :: riverstate  !<River states
    INTEGER,INTENT(INOUT) :: freezeupday(2)
...
! 4) use any remaining heat and the zero degree melt water to update the river state
  ...
  !----------------------------------------------------------
  SUBROUTINE calculate_lake_epidepth(lakearea,prec,evapl,qinmm,epidepth)
  ...
  END SUBROUTINE calculate_lake_epidepth
  ...
  SUBROUTINE ice_processes_in_lake(i,itype,iluse,snowfall,temp,lakesurftemp,  &
                                   ...
                                   breakupday,lakearea,epidepth,freezeuparea)
  ...
  SUBROUTINE calculate_lakeice_lakewater_interaction(itype,i,frozenstate,lakestate,dlakewidt,epidepth,breakupday)
    ...
    REAL, INTENT(IN)   :: epidepth    !<epilimnion depth (m)
    INTEGER, INTENT(INOUT) :: breakupday(2)
