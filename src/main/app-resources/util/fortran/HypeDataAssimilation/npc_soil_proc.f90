...
!    areaij=classbasin(i,j)%part*basin(i)%area*1.E-6   !(km2)
      ...
      IF(part>0)THEN
        ...
        IF(cropdata(k)%resdayno==0 .OR.   &
          ...
          IF(thickness(2)>0)THEN
            ...
            common_cres(1,1) = common_cres(1,1) + part * cropdata(k)%rescamount * (1. - cropdata(k)%resdown) / littdays
  ...
  SUBROUTINE soil_np_processes(i,j,iluse,calcN,calcP,calcC,dayno,area,wp,fc,ep, &
    ...
    REAL, INTENT(IN)    :: hsatINpar           !<model parameter half saturation IN of soil (mg/L)
    ...
    !  denitrification = 0.
    !  denpar = denpar1
    !  IF(thickness(1)>0.) CALL soil_denitrification(wp(1)+fc(1)+ep(1),denpar,hsatINpar,soilstate%water(1,j,i),soilstate%temp(1,j,i),soilstate%conc(:,1,j,i),sink)
    !  denitrification(1) = sink(i_in)
    !  IF(thickness(2)>0.) CALL soil_denitrification(wp(2)+fc(2)+ep(2),denpar,hsatINpar,soilstate%water(2,j,i),soilstate%temp(2,j,i),soilstate%conc(:,2,j,i),sink)
    ...
    !    IF(wqregpar(m_denitr3reg,basin(i)%wqparregion)>0.) denpar=wqregpar(m_denitr3reg,basin(i)%wqparregion)
    !    CALL soil_denitrification(wp(3)+fc(3)+ep(3),denpar,hsatINpar,soilstate%water(3,j,i),soilstate%temp(3,j,i),soilstate%conc(:,3,j,i),sink)
  ...
  SUBROUTINE calculate_soil_profile_denitrification(calcN,i,j,iluse,wp,fc,ep,thickness,denpar_basic,soilstate,denitrification)
    ...
    IF(calcN)THEN
      ...
      IF(thickness(3)>0.)THEN
        IF(landpar(m_denitr3,iluse)>0.) denpar=landpar(m_denitr3,iluse)
        IF(landpar(m_denitr3,iluse)<0.) denpar=0.
        IF(wqregpar(m_denit3reg,basin(i)%wqparregion)>0.) denpar=wqregpar(m_denit3reg,basin(i)%wqparregion)
