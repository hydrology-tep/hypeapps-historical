...
CONTAINS
  ...
  SUBROUTINE load_ascii_qx_observations(dir,ns,ndt,bdate,edate,n_Result) 
    ...
    IF(readoutregion)THEN
      ...
      IF(xorex)THEN
        CALL count_data_cols(fileunit_temp,TRIM(dir)//filename_Xoreg,1, xorcol,n_Result)
        IF(n_Result.NE.0) RETURN
        xorcol = xorcol-1
        IF(xorcol>0)THEN
          IF(.NOT.ALLOCATED(xorvar)) ALLOCATE(xorvar(xorcol,2))
          CALL check_xobs(fileunit_temp,TRIM(dir)//filename_Xoreg,xorcol,    &
                 max_outvar,outvarid(:)%shortname,xorvar,n_Result) 
          IF(n_Result.NE.0) RETURN
        ENDIF
      ENDIF      
    ENDIF
    ...
    IF(readoutregion)THEN
      IF(xorex .AND. xorcol>0)THEN
        CALL prepare_read_matrix(fileunit_xoreg,TRIM(dir)//filename_Xoreg,3,bxrdate,n_Result)
...
900 WRITE(6,*) 'Error open file: ', TRIM(filepath)
  ...
  SUBROUTINE reset_observations(dir,n_Result) 
    ...
    CALL reload_modeldefined_observations(dir,n_Result)
    IF(n_Result.NE.0) RETURN
  ...
  USE MODELMODULE, ONLY : load_modeldefined_input, &
  ...
  !>
  !>/b Consequences Module modvar variables glacierexist 
...
!    numaqu = MAXVAL(xi(:,aquid_i))
    ...
    DO i = 1,nrows
      ...
      ELSEIF(xi(i,subid_i)==0)THEN  !Row with aquifer information
        ...
        aquifer(aqindex(xi(i,aquid_i)))%parregion = xi(i,reg_i)
  ...
  SUBROUTINE read_and_calc_basindata(funit,infile,n,lakedataid,maxcol,mcols) 
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:10)=='mrwl0     ') code(i) = i_real   !main river reference elevation (m.a.s.l) for correcting main river water depth to main river water elevation (m.a.s.l)
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:10)=='mrwl0     ')  basin(1:n)%mrwl0        = xr(1:n,rindex(i))
    ...
    IF(readsfobs) sfobsid(1:n) = basin(1:n)%subid      
    IF(readswobs) swobsid(1:n) = basin(1:n)%subid      
    IF(readwind) uobsid(1:n) = basin(1:n)%subid      
    IF(readhumid) rhobsid(1:n) = basin(1:n)%subid      
    IF(readtminmaxobs)THEN
...
!--------------------------------------------------------------------
  ...
  SUBROUTINE load_forcing_information_data(funit,dir,ns,status) 
    ...
    OPEN(UNIT=funit,FILE=TRIM(infile),STATUS='old',ACTION='read',ERR=900)
      ...
      !Check if some id for forcing is lacking, use default value
      IF(SUM(pobsid(1:ns))==0) pobsid(1:ns) = basin(1:ns)%subid
      IF(SUM(tobsid(1:ns))==0) tobsid(1:ns) = basin(1:ns)%subid
      IF(readsfobs)THEN
        IF(SUM(sfobsid(1:ns))==0) sfobsid(1:ns) = basin(1:ns)%subid      
      ENDIF    
      IF(readswobs)THEN
        IF(SUM(swobsid(1:ns))==0) swobsid(1:ns) = basin(1:ns)%subid      
      ENDIF
      IF(readwind)THEN
        IF(SUM(uobsid(1:ns))==0) uobsid(1:ns) = basin(1:ns)%subid      
      ENDIF
      IF(readhumid)THEN
        IF(SUM(rhobsid(1:ns))==0) rhobsid(1:ns) = basin(1:ns)%subid      
      ENDIF
      IF(readtminmaxobs)THEN
        IF(SUM(tminobsid(1:ns))==0) tminobsid(1:ns) = basin(1:ns)%subid      
        IF(SUM(tmaxobsid(1:ns))==0) tmaxobsid(1:ns) = basin(1:ns)%subid      
      ENDIF    
...
900 WRITE(6,*) 'Error open file: ', TRIM(infile)
  ...
  SUBROUTINE load_cropdata(dir,infile,n,n_Result) 
    ...
    IF(.NOT.(calcNP .OR. i_t1>0)) DEALLOCATE(cropdata)
    sumirr = SUM(cropirrdata(1:n)%plantingdayno)
    IF(sumirr==0) DEALLOCATE(cropirrdata)

    IF(calcNP .OR. (i_t1>0) .OR. (sumirr>0))THEN
...
!    INTEGER ioutregion
!    INTEGER readregions(maxoutbasins)
    ...
    INTEGER basinoutputvar(max_typeofperiods,max_outvar,3)
    INTEGER regionoutputvar(max_typeofperiods,max_outvar,2)
    INTEGER mapoutputvar(max_typeofperiods,max_outvar,3)
    INTEGER timeoutputvar(max_typeofperiods,max_outvar,3)
...
25      CONTINUE
        ...
        IF(code2(1:8)=='variable')THEN
          DO
            ...
            !  areagg = 1
            !  n_Result = find_variable_index(varstr(3:6),varindex,flowtype)
            !ELSEIF(varstr(1:2)=='rg')THEN
            !  areagg = 2
            !  n_Result = find_variable_index(varstr(3:6),varindex,flowtype)
            !ELSE
            !  areagg = 0
            !  n_Result = find_variable_index(varstr(1:4),varindex,flowtype)
            !ENDIF
            n_Result = find_variable_index_type(varstr,varindex,flowtype,areagg)
            ...
            IF(code(1:11)=='basinoutput')THEN
              ...
              basinoutputvar(icheck,ibasinoutputvar(icheck),2) = flowtype
              basinoutputvar(icheck,ibasinoutputvar(icheck),3) = areagg
            ELSEIF(code(1:9)=='mapoutput')THEN
              ...
              mapoutputvar(icheck,imapoutputvar(icheck),2) = flowtype
              mapoutputvar(icheck,imapoutputvar(icheck),3) = areagg
            ELSEIF(code(1:10)=='timeoutput')THEN
              ...
              timeoutputvar(icheck,itimeoutputvar(icheck),2) = flowtype
              timeoutputvar(icheck,itimeoutputvar(icheck),3) = areagg
            ELSEIF(code(1:12)=='regionoutput')THEN
              ...
              IF(iregionoutputvar(icheck)>max_outvar)THEN
                ...
                EXIT
              ENDIF
              regionoutputvar(icheck,iregionoutputvar(icheck),1) = varindex
              regionoutputvar(icheck,iregionoutputvar(icheck),2) = flowtype
            ENDIF
          ENDDO
          IF(exitbigloop)EXIT
          IF(cyclebigloop)CYCLE
        ...
        !      exitbigloop = .TRUE.
        !    ENDIF  
        !    ioutregion = ioutregion + 1
        !    IF(ioutregion>maxoutbasins)THEN
        !      WRITE(6,*) 'Warning: Number of "regionoutput" "outregion"s larger than max',maxoutbasins
        !      WRITE(6,*) 'Warning: Excess output regions will be ignored'
        !    ELSE
        !      readregions(ioutregion) = intvalue
        !    ENDIF
        !  ENDDO
        !  IF(exitbigloop)EXIT
        !  IF(cyclebigloop)CYCLE
      ...
      IF(code(1:4)=='crit')THEN
        ...
        ELSE
          ...
          IF(code2(1:9)=='cvariable')THEN
            ...
            ENDIF
            n_Result = find_variable_index_type(varstr,varindex,flowtype,areagg)
            !n_Result = find_variable_index(varstr(1:4),varindex,flowtype)   !Not adjusted to be used for up or rg-variales yet
            IF(n_Result>0)THEN
...
200 CONTINUE
    ...
    DO
      ...
      temp = set_output(1,io,noutput,max_outvar,basinoutputvar(i,:,1),    &
         basinoutputvar(i,:,2),ibasinoutputvar(i),basinoutputperiod(i),    &
         basinoutputdecimal(i),basinoutputsignif(i),basinoutputvar(i,:,3),maxoutbasins,ioutbasin(i),readbasins(i,:))
      i=i+1
    ENDDO
    i=1
    DO
      ...
      temp = set_output(2,io,noutput,max_outvar,mapoutputvar(i,:,1),    &
         mapoutputvar(i,:,2),imapoutputvar(i),mapoutputperiod(i),    &
         mapoutputdecimal(i),mapoutputsignif(i),mapoutputvar(i,:,3))
      i=i+1
    ENDDO
    i=1
    DO
      ...
      temp = set_output(3,io,noutput,max_outvar,timeoutputvar(i,:,1),    &
         timeoutputvar(i,:,2),itimeoutputvar(i),timeoutputperiod(i),    &
         timeoutputdecimal(i),timeoutputsignif(i),timeoutputvar(i,:,3))
      i=i+1
    ENDDO
    i=1
    DO
      ...
      io=io+1
      temp = set_output(4,io,noutput,max_outvar,regionoutputvar(i,:,1),   &
         regionoutputvar(i,:,2),iregionoutputvar(i),regionoutputperiod(i),       &
         regionoutputdecimal(i),regionoutputsignif(i))
      i=i+1
    ENDDO
    ...
    noutvar = SUM(iregionoutputvar)   !Hur ska jag få reda på denna!?
!    temp = set_output(4,max_outvar,1,1,varregionoutput(:,1),   &    !set region output
!         varregionoutput(:,2),(/1/),ivarregionoutput,1,regionoutputperiod,       &
!         regionoutputdecimal,regionoutputsignif)
    !noutput=noutput+1
    !DO io=1,noutput-1
    !  temp = set_output(4,max_outvar,io,noutput,varregionoutput(io,:,1),   &    !set region output
    !     varregionoutput(io,:,2),(/1/),ivarregionoutput(io),1,regionoutputperiod(io),       &
    !     regionoutputdecimal(io),regionoutputsignif(io))
    !ENDDO
    !noutput=noutput+1
!    io=noutput  !One basinoutput
!    DO io=noutput+1,noutput+1
      !temp = set_output(1,max_outvar,io,noutput,varbasinoutput(:,1),    &
      !   varbasinoutput(:,2),(/1/),ivarbasinoutput,1,basinoutputperiod,    &
      !   basinoutputdecimal,basinoutputsignif)
    temp = set_outvar(noutput)  !Denna behöver inte sättas här! Vill ha med crit och assim
    temp = set_outvar_crit()
    !temp = set_output(4,max_outvar,1,noutput,varregionoutput(1,:,1),   &    !set region output
    !     varregionoutput(1,:,2),(/1/),ivarregionoutput(1),1,regionoutputperiod(1),       &
    !     regionoutputdecimal(1),regionoutputsignif(1))
    !temp = set_output(4,max_outvar,2,noutput,varregionoutput(2,:,1),   &    !set region output dubblet på årsmedel
    !     varregionoutput(2,:,2),(/1/),ivarregionoutput(2),1,4,       &
    !     regionoutputdecimal(2),regionoutputsignif(2))
    !temp = set_output(4,max_outvar,3,noutput,varregionoutput(:,1),   &    !set region output dubblet på årsmedel
    !     varregionoutput(:,2),(/1/),ivarregionoutput,1,2,       &
    !     regionoutputdecimal,regionoutputsignif)
    !temp = set_output(4,max_outvar,4,noutput,varregionoutput(:,1),   &    !set region output dubblet på årsmedel
    !     varregionoutput(:,2),(/1/),ivarregionoutput,1,3,       &
    !     regionoutputdecimal,regionoutputsignif)
    !noutreg = 1
    !temp = set_outregion(4,noutreg,max_outvar,(/1/),vartimeoutput(:,1),   &    !read/save regions for region output
    !     REAL(vartimeoutput(:,2)),(/2./),(/5./))
    !nsubout = ioutbasin
    !IF(ioutbasin>0)THEN      
    !  ALLOCATE(temp_outbasins(ioutbasin))
    !  temp_outbasins=readbasins(1:ioutbasin)
    !ENDIF
!    nsubout = MAXVAL(ibasinoutputvar)   !temporary may be increased to nsub
    ...
    !IF(ioutbasin>0)THEN
    !  WRITE(6,'(I5,a,I3,a)')ivarbasinoutput,' output variables are asked to be saved for ',ioutbasin,' subbasins'
    !ELSEIF(outvarallbasin)THEN
    !  WRITE(6,'(a)')ivarbasinoutput,'output variables will be saved for all subbasins'
    !ENDIF
    !IF(ivarmapoutput>0) WRITE(6,'(I5,a52)')ivarmapoutput,' output variables will be saved as files for mapping'
    !IF(ivartimeoutput>0) WRITE(6,'(I5,a)')ivartimeoutput,' output variables will be saved as timeseries output'
    !IF(ioutregion>0) WRITE(6,'(I5,a,I3,a)')SUM(ivarregionoutput),' output variables are asked to be saved for ',ioutregion,' output regions'
...
800 CONTINUE
  ...
  !------------------------------------------------------------------------
  INTEGER FUNCTION set_outvar(nout)

    USE WORLDVAR, ONLY : output, & !OUT
                         outvarinfotemp !OUT
    USE MODVAR, ONLY : noutvar, &
                       max_noutvar, &
                       max_outvar, &
                       outvarindex  !OUT
    ...
    DO iout = 1,nout
      ...
      DO i = 1,output(iout)%nvar
        CALL set_outvar_for_variable(output(iout)%variable(i)%idindex,output(iout)%variable(i)%areaagg,output(iout)%variable(i)%ovindex)
      ENDDO
    ENDDO
  ...
  !------------------------------------------------------------------------
  INTEGER FUNCTION set_outvar_crit()
    ...
    DO ia = 1,nacrit
      CALL set_outvar_for_variable(acccalvar(ia)%comp,acccalvar(ia)%areaagg,acccalvar(ia)%compoutvar)
      CALL set_outvar_for_variable(acccalvar(ia)%rec,acccalvar(ia)%areaagg,acccalvar(ia)%recoutvar)
!      CALL set_outvar_for_variable(acccalvar(ia)%comp,0,acccalvar(ia)%compoutvar)
!      CALL set_outvar_for_variable(acccalvar(ia)%rec,0,acccalvar(ia)%recoutvar)
    ENDDO
  ...
  USE MODVAR, ONLY : CROPDATATYPE,  &
    ...
    TYPE(CROPDATATYPE),ALLOCATABLE,INTENT(INOUT) :: cropdata(:)
    TYPE(CROPIRRDATATYPE),ALLOCATABLE,INTENT(INOUT) :: cropirrdata(:)
  ...
  !------------------------------------------------------------------------
  SUBROUTINE set_outvar_for_variable(varid,areaagg,ovindex)
    
    USE MODVAR, ONLY : outvarindex, &
                       outvarid, &
                       noutvar  !OUT
    USE WORLDVAR, ONLY : OUTVARIABLETYPE, &
                         outvarinfotemp !OUT
    ...
    IF(.NOT.found)THEN
      ...
      ovindex = icurrent
      outvarinfotemp(icurrent) = OUTVARIABLETYPE(idindex,outvarid(idindex)%vartype,areaagg,ioutvar,0)
      ...
      IF(outvarid(idindex)%water>0)THEN
        ...
        IF(.NOT.found2)THEN
          ...
          outvarinfotemp(icurrent2) = OUTVARIABLETYPE(idindex,outvarid(idindex)%vartype,areaagg,ioutvar,0)
          ...
          IF(outvarinfotemp(icurrent2)%areaagg>0)THEN
            ...
            IF(.NOT.found3)THEN
              ...
              outvarinfotemp(ioutvar) = OUTVARIABLETYPE(idindex,outvarid(idindex)%vartype,0,ioutvar,0)
            ENDIF
          ENDIF
        ENDIF
      ENDIF
      ...
      IF(outvarinfotemp(icurrent)%areaagg>0)THEN
        ...
        IF(.NOT.found3)THEN
          ...
          outvarinfotemp(icurrent2) = OUTVARIABLETYPE(idindex,outvarid(idindex)%vartype,0,ioutvar,0)
          IF(outvarid(idindex)%water>0)THEN
            found4 = .FALSE.
            DO i2=1,ioutvar-1
              IF(outvarinfotemp(i2)%idindex == outvarid(idindex)%water)THEN
    ...
    DO io=1,noutput
      ...
      ELSE
        ...
        ELSEIF(output(io)%fileformat==4)THEN
          ...
          DO ia = 1,na
            DO ireg = 1,noutreg
              IF(tempoutareas(ia)==outregion(ireg)%outregid)THEN
                iaccept = iaccept + 1
  ...
  SUBROUTINE load_output_regions(dir,nreg,status)
    ...
    !Allocate and initiate local variables
    IF(.NOT.ALLOCATED(xi)) ALLOCATE(xi(nrows,ncols))     ! table for integer data from file
    IF(.NOT.ALLOCATED(code)) ALLOCATE(code(ncols))       ! vector for coded info from file
    IF(.NOT.ALLOCATED(rindex)) ALLOCATE(rindex(ncols))   ! vector with indices for columns with real data
    IF(.NOT.ALLOCATED(iindex)) ALLOCATE(iindex(ncols))   ! vector with indices for columns with integer data
    IF(.NOT.ALLOCATED(xr)) ALLOCATE(xr(nrows,ncols))     ! table for real data from file
    IF(.NOT.ALLOCATED(str)) ALLOCATE(str(ncols))         ! vector for text info from file
    ALLOCATE(readsubid(nsub,nrows))   !nsub = nsub_base  ! table with subids for each region
    ALLOCATE(readweight(nsub,nrows))                     ! table with subbasin weights for each region
    readsubid = 0
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='area      ') code(i) = i_real
      IF(str(i)(1:10)=='obsfunc   ') code(i) = i_intg
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='area      ') outregion(1:nrows)%area     = xr(1:nrows,rindex(i))
      IF(str(i)(1:10)=='obsfunc   ') outregion(1:nrows)%obsfunc  = xi(1:nrows,iindex(i))
      ...
      IF(output(io)%fileformat==2)THEN
        DO ivar = 1,output(io)%nvar
          IF(resultseq.AND.simsequence>0 .OR. runens)THEN  !Ex. mapCOUT_005.txt
            ...
            IF(runens) nn=iens
            filename(1:7) = outvarid(output(io)%variable(ivar)%idindex)%mapfilename(1:7)
            filename(8:8) = '_'
            IF(allens)THEN
            ...
            ENDIF
          ELSE  !Ex. mapCOUT.txt
            filename=outvarid(output(io)%variable(ivar)%idindex)%mapfilename
          ENDIF
601    FORMAT(I3.3)
602    FORMAT(I7.7)
          ...
          !Write heading
          IF(writematlab)THEN
            WRITE(fileunit_temp,*) '%This is a file with '//TRIM(var)//' in '//TRIM(unit)//' for GIS mapping'
          ELSE
            WRITE(fileunit_temp,*) 'This is a file with '//TRIM(var)//' in '//TRIM(unit)//' for GIS mapping'
          ENDIF
          outtxt = ''
  ...
  !--------------------------------------------------------------------
  SUBROUTINE write_subbasin_assessment(dir,numsub,n,sas2,iens,runens)
    ...
    REAL, INTENT(IN)    :: sas2(numsub,maxsubass,n)  !<Subbasin assessment
    INTEGER, INTENT(IN) :: iens                   !<Current simulation
    LOGICAL, INTENT(IN) :: runens                 !<Flag for ensemble simulation
...
601   FORMAT(I3.3)
      ...
      IF(writematlab)THEN
          WRITE(fileunit_temp,'(a55,I2,a13,a4,a2,a4,a8,a6)')    &
      ...
      ELSE
          WRITE(fileunit_temp,'(a56,I2,a13,a4,a2,a4,a8,a6)')    &
                '!!Subbasin assessment. Criteria is calculated for period',calvarper,'. Variables: ',outvarid(acccalvar(i)%rec)%shortname,', ',outvarid(acccalvar(i)%comp)%shortname,' Unit: ',outvarid(acccalvar(i)%comp)%shortunit
        WRITE(fileunit_temp,'(a)') 'SUBID'//CHAR(9)//'NSE'//CHAR(9)//'CC'//CHAR(9)//'RE(%)'//CHAR(9)//'RSDE(%)'//CHAR(9)//'Sim'//CHAR(9)//'Rec'//CHAR(9)//'SDSim'//CHAR(9)//'SDRec'//CHAR(9)//'MAE'//CHAR(9)//'RMSE'//CHAR(9)//'Bias'//CHAR(9)//'SDE'//CHAR(9)//'KGE'//CHAR(9)//'KGESD'//CHAR(9)//'KGEM'//CHAR(9)//'NRMSE'//CHAR(9)//'NSEW'
      ENDIF
      ELSEIF(acccalvar(i)%areaagg==2)THEN
        IF(writematlab)THEN
          WRITE(fileunit_temp,'(a55,I2,a15,a4,a4,a4,a8,a6)')    &
                '%Subbasin assessment. Criteria is calculated for period',calvarper,'. Variables: rg',outvarid(acccalvar(i)%rec)%shortname,', rg',outvarid(acccalvar(i)%comp)%shortname,' Unit: ',outvarid(acccalvar(i)%comp)%shortunit
        ...
        ELSE
          WRITE(fileunit_temp,'(a56,I2,a15,a4,a4,a4,a8,a6)')    &
                '!!Subbasin assessment. Criteria is calculated for period',calvarper,'. Variables: rg',outvarid(acccalvar(i)%rec)%shortname,', rg',outvarid(acccalvar(i)%comp)%shortname,' Unit: ',outvarid(acccalvar(i)%comp)%shortunit
          WRITE(fileunit_temp,'(a)') 'OUTREGID'//CHAR(9)//'NSE'//CHAR(9)//'CC'//CHAR(9)//'RE(%)'//CHAR(9)//'RSDE(%)'//CHAR(9)//'Sim'//CHAR(9)//'Rec'//CHAR(9)//'SDSim'//CHAR(9)//'SDRec'//CHAR(9)//'MAE'//CHAR(9)//'RMSE'//CHAR(9)//'Bias'//CHAR(9)//'SDE'//CHAR(9)//'KGE'//CHAR(9)//'KGESD'//CHAR(9)//'KGEM'//CHAR(9)//'NRMSE'//CHAR(9)//'NSEW'
        ENDIF
      ENDIF
      ...
      ENDDO
        !DO isub=1,numsub-nsub
        !  x=sas2(nsub+isub,1:maxsubass,i)
        !  IF(x(1)/=missing_value) CALL write_dataline(fileunit_temp,maxsubass,x,ndec,0,0,CHAR(9),0,writematlab,id=outregion(isub)%outregid)
        !ENDDO
      ENDIF
      CLOSE(fileunit_temp)
...
601 FORMAT(I3.3)
  ...
  SUBROUTINE open_subbasinfiles(dir,n,iens,runens,allens)
    ...
    CHARACTER(LEN=17) str       !part of filename
    CHARACTER(LEN=5) var
    CHARACTER(LEN=6) var2
    CHARACTER(LEN=6) varunit
    INTEGER lt,lout,ls,lout1,lout2
    ...
    DO io=1,noutput
      IF(output(io)%fileformat==1)THEN
        ...
        DO i = 1,nselected
          ...
          !Create filename
          str=''
          WRITE(str(1:7),600) basin(isb)%subid
          !WRITE(str2,603) output(io)%period
          lout=7
          IF(ispecific>1)THEN
            str(8:8) = '_'
            str(9:10) = TRIM(ADJUSTL(outperiodname(output(io)%period)))
            lout=10
          ENDIF
          IF(resultseq.AND.simsequence>0 .OR. runens)THEN  !Ex. 0000124_005.txt
            ...
            str(lout+1:lout+1) ='_'
            IF(allens)THEN
              WRITE(str(lout+2:lout+8),602) nn   !Ex. 0000124_0001001.txt
            ELSE
              WRITE(str(lout+2:lout+4),601) nn
            ENDIF
          ENDIF
          !Open and write heading to file
          OPEN(UNIT=funit,FILE=TRIM(dir)//TRIM(ADJUSTL(str))//'.txt',status='unknown',form='formatted')
          WRITE(funit,'(a)') outtxt(1:lout1)     !Write heading
    ...
    DO io=1,noutput
      IF(output(io)%fileformat==4)THEN
        ...
        DO i = 1,noutreg
          ...
          !Create filename
          str=''
          WRITE(str(1:7),600) outregion(i)%outregid
          lout=7
          IF(ispecific>1)THEN
            str(8:8) = '_'
            str(9:10) = TRIM(ADJUSTL(outperiodname(output(io)%period)))
          ...
          ENDIF
          !Open and write heading to file
          OPEN(UNIT=funit,FILE=TRIM(dir)//TRIM(str)//'.txt',status='unknown',form='formatted')
...
603 FORMAT(I1.1)
  ...
  SUBROUTINE write_regionfiles(io,idt,ndt,iens,cd)
    ...
    ALLOCATE(x(output(io)%nvar))
    DO ir = 1,noutreg
      !Calculate all defined regions, (may print only selected later)
      CALL compute_regionoutput(cd,io,ir,output(io)%nvar,idt,ndt,1,pwrite,x)
      IF(pwrite) CALL write_dataline(fileunit_add_sub+nsub*(io-1)+ir+(iens-1)*fileunit_add_dabout,output(io)%nvar,x,output(io)%decimal,  &
                      output(io)%signfig,output(io)%period,CHAR(9),0,writematlab,d=cd,odate=outstartdate)
    ENDDO
    IF(ALLOCATED(x)) DEALLOCATE(x)
  ...
  SUBROUTINE open_timefiles(dir,n,iens,runens,allens)
    ...
    INTEGER funit
    INTEGER ispecific
    CHARACTER(LEN=10) format_str,commlength
        ...
        DO ivar = 1,output(io)%nvar
          ...
          IF(resultseq.AND.simsequence>0 .OR. runens)THEN  !Ex. timeCOUT_005.txt
            ...
            IF(output(io)%variable(ivar)%areaagg==2)THEN    !region output
              filename(1:4) = outvarid(output(io)%variable(ivar)%idindex)%timefilename(1:4)
              filename(5:6) = 'RG'
              filename(7:10) = outvarid(output(io)%variable(ivar)%idindex)%timefilename(5:8)
              lout=10
            ELSEIF(output(io)%variable(ivar)%areaagg==1)THEN    !upstream output
              ...
              lout=10
            ELSE
              filename(1:8) = outvarid(output(io)%variable(ivar)%idindex)%timefilename(1:8)
              lout=8
            ENDIF
            IF(ispecific>1)THEN
              filename(lout+1:lout+1) = '_'
              filename(lout+2:lout+3) = TRIM(ADJUSTL(outperiodname(output(io)%period)))
              lout=lout+3
            ENDIF
            filename(lout+1:lout+1) = '_'
            IF(allens)THEN
              WRITE(filename(lout+2:lout+8),602) nn   !Ex. timeCOUT_0001001.txt
              filename(lout+9:lout+12) = '.txt'
            ELSE
              WRITE(filename(lout+2:lout+4),601) nn
              filename(lout+5:lout+8) = '.txt'
            ENDIF
...
602    FORMAT(I7.7)
          ...
          IF(writematlab)THEN
            ...
            format_str=''
            WRITE(commlength,'(I10)') LEN(TRIM(commentheading))   !Integer to character
            format_str = '(A'//TRIM(ADJUSTL(commlength))//')'
            WRITE(funit,format_str) commentheading
...
100 WRITE(6,*) 'End of file during read optpar.txt for river dimension' 
  ...
  !> accdata_basinload, maptime, tmap, 
  !> output are set.
  !------------------------------------------------------------------
  ...
  SUBROUTINE allocate_outvar(n,nj,ns,nout)
    ...
    USE MODVAR, ONLY : outvar,max_noutvar,conductload,outvar_classload,outvar_basinload, max_classoutvar,max_basinoutvar
      ...
      IF(outvarinfo(iout)%areaagg==2)THEN
        DO ir = 1,noutreg
          IF(xoregindex(outvarinfo(iout)%idindex,ir)>0)THEN
            y=xoregobsi(xoregindex(outvarinfo(iout)%idindex,ir))          !regional variable exists in xoregobs.txt
          ELSE
            CALL calculate_region_average(ir,outvarinfo(iout)%ovindex0,y) !calculate regional variable from subbasin outvar data
          ENDIF
  ...
  SUBROUTINE checkindata_part1()
    ...
    IF(modeloption(p_growthstart)==1)THEN
      ...
      !IF(basinarea<0) WRITE(6,*) 'Negative latitudes given. Check GeoData.txt'  !Negative latitudes ok.
    ENDIF
...
100 CONTINUE
  ...
  SUBROUTINE checkindata_part2() 
    ...
    IF(forcid<=0)THEN
      WRITE(6,*) 'Identification number of temperature forcing is zero for some subbasin. Check ForcKey.txt'
      checkdata(2,2) = .TRUE.
    ENDIF
    forcid = MINVAL(pobsid)
    IF(forcid<=0)THEN
      WRITE(6,*) 'Identification number of precipitation forcing is zero for some subbasin. Check ForcKey.txt'
      checkdata(2,2) = .TRUE.
    ENDIF
    IF(readsfobs)THEN
      forcid = MINVAL(sfobsid)
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of snowfall forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
    ENDIF    
    IF(readswobs)THEN
      ...
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of shortwave radiation forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
    ENDIF
    IF(readwind)THEN
      ...
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of wind speed forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
    ENDIF
    IF(readhumid)THEN
      ...
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of relative humidity forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
    ENDIF
    IF(readtminmaxobs)THEN
      ...
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of min temperature forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
      forcid = MINVAL(tmaxobsid)
      IF(forcid<=0)THEN
        WRITE(6,*) 'Identification number of max temperature forcing is zero for some subbasin. Check ForcKey.txt'
        checkdata(2,2) = .TRUE.
      ENDIF
    ENDIF
    ...
    IF(n_Result/=0)THEN
      WRITE(6,*) 'Error: Missmatch in observations between model and Pobs.txt. Check ForcKey and Pobs.txt.'
      checkdata(2,2)=.TRUE.
    ENDIF 
