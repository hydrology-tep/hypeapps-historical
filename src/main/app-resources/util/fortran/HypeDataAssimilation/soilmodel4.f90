...
!       dayno, & !use pseudo_dayno as argument instead
       currentdate, &
       xobsi,xobsindex, &
  ...
  USE HYPEVARIABLES, ONLY : wpmm,fcmm,epmm, &
       ...
       m_minerfn,m_minerfp,m_degradhn,m_degradhp,m_denitrlu,m_hsatINsoil, &
...
CONTAINS
  ...
  !----------------------------------------------------------------
  SUBROUTINE soilmodel_4(i,j,isoil,iluse,subid,classarea,prec,cprec,temp, & 
       daylength,rrcscorr,phoscorr,cevpcorr,incorr,oncorr,dayno,  &
       frozenstate,soilstate,miscstate,surfaceflow,csrunoff,   &
       crunoffd,cropuptakein,nitrif,denitrif,epot,gwat,frostdepth,snowcov,    &
       smdef,evap,cevap,crunoff1,crunoff2,crunoff3,pwneed,irrappl,irrsources,  &
       nonflood_part,snowfall, rainfall,cropsources,ruralaload,atmdepload1,atmdepload,sffrac,swrad,  &
       radext,netrad,actvap,satvap,wind,infiltrationflows,floodplainflows,evapflows,  &
       runofflows,verticalflows,cverticalflows,horizontalflows,horizontalflows2,evapsnow,cevapsnow,totalsoilrunoff,cruralflow)  
    ...
    REAL, INTENT(OUT) :: cevapsnow(numsubstances) ! conc of snow evaporation
    ...
    gwat=0.; smdef=0.
    frostdepth = 0.
    horizontalflows=0.;horizontalflows2=0.
    verticalflows=0.
    evapflows = 0.
    snowcov = 0.
    atmdepload1 = 0.
    ...
    IF(ffparea<classfparea)THEN
      ...
      CALL calculate_soil_runoff(i,j,subid,wpmm(:,j),fcmm(:,j),epmm(:,j), &
              soildepth(:,j),soilthick(:,j),nonflood_part*streamdepth(j), &
      ...
      CALL calculate_tile_drainage(i,j,isoil,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),   &
              soildepth(:,j),soilthick(:,j),nonflood_part*tiledepth(j),rrcscorr,soilstate,runoffd, &
              crunoffd,cweights)
      runofflows(4:6) = runoffd*cweights(1:3)
      
      !Add load from local diffuse sources to the lowest soil layer
      CALL local_diffuse_source(i,j,pwmm,classfparea*1.E-6,soilstate,ruralaload,verticalflows(3:4),horizontalflows(1:3),cruralflow)
      ...
      CALL infiltration(i,j,isoil,wpmm(:,j),fcmm(:,j),ginfilt,cginfilt,infilt,  &
      ...
      surfaceflow = satoverflow + excessinfilt*nonflood_part
      IF(surfaceflow > 0.) THEN
        csrunoff(:) = (soilstate%conc(:,1,j,i) * satoverflow + excessinfilt * cexcessinfilt(:)*nonflood_part) / surfaceflow     !used for satoverflow and excessinfilt
      ENDIF
    ...
    !Erosion of particulate phosphorus with fastflow (surface flow and macropore flow) including delay in temporary storage.
      IF(i_pp>0) CALL runoff_pp_by_erosion(i,j,isoil,iluse,dayno,rainfall,surfaceflow,   &
              ...
              crunoff2(i_pp),crunoff3(i_pp),snowonfp,soilstate)  
      ...
      CALL soil_np_processes(i,j,iluse,conductN,conductP,conductC,dayno,classarea,wpmm(:,j),  &
           ...
           (2.-incorr) * landpar(m_denitrlu,iluse),genpar(m_hsatINsoil),landpar(m_degradhp,iluse),soilstate)
      ...
      IF(i_t1>0)THEN
        IF(T1release>0.)THEN
          IF(surfaceflow>0.)THEN
            csrunoff(i_t1) = csrunoff(i_t1) + (T1release * (surfaceflow/(infilt*nonflood_part+surfaceflow))) / surfaceflow
          ENDIF
          soilstate%partT1(1,j,i) = soilstate%partT1(1,j,i) + T1release * (infilt*nonflood_part/(infilt*nonflood_part+surfaceflow))
        ENDIF
      ENDIF      
      ...
      IF(i_t2>0) THEN
        ...
        csoilrunoff(i_t2,1) = trunofftemp(1)    !dessa onödiga
    ...
    IF(miscstate%floodwater(watertype,i)>0)THEN
      ...
      ENDIF
        CALL flood_infiltration(i,j,pwmm(1),netaddtosoil,addedtosoil,miscstate%cfloodwater(:,watertype,i),soilstate) 
    ...
    IF(surfaceflow>0.)THEN
      CALL add_water(nc,miscstate%floodwater(watertype,i),miscstate%cfloodwater(:,watertype,i),surfaceflow*1.E-3*classfparea,csrunoff)
    ENDIF
    IF(totalsoilrunoff(watertype)>0.)THEN  !ska nog flytta denna till å/sjövolym istället, det är rundgång!
