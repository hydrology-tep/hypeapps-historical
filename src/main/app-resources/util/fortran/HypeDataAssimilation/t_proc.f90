...
MODULE TRACER_PROCESSES
  ...
  !> tracer (T1) - Processes)
  !-----------------------------------------------------------------
  SUBROUTINE tracer_processes_in_lake(i,itype,lakearea,lakestate)
    ...
    IF(i_t1>0)THEN
      ...
      CALL sedimentation(lakestate%slowwater(itype,i)*lakearea*1.E-3,lakestate%concslow(i_t1,itype,i),genpar(m_t1sed),lakearea)
    ENDIF
  ...
  SUBROUTINE sorption_of_tracer(vol,conc,sorbed,layerthick)
    ...
    !Local variables
    REAL totalT1, sorbed_conc0, xn_1,coeff, equi_sorbed, adsdes
    ...
    IF(i_t1>0)THEN
      IF((vol>0. .AND. conc(i_t1)>0.) .OR. sorbed>0.)THEN
        ...
        xn_1 = totalT1 / (vol + coeff)
        equi_sorbed = genpar(m_T1freuc) * xn_1
  ...
  !> Move sorbed T1 from above-soil pool to soil pool (ploughing)
  !>
  !> \b Reference ModelDescription Chapter Tracer simulation (General 
  !> tracer (T1) - Sources)
  !-----------------------------------------------------------------
