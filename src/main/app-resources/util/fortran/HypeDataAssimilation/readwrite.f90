...
MODULE READWRITE_ROUTINES
  ...
  !----------------------------------------------------------------------------
  SUBROUTINE check_pt_stn(funit,infile,ns,geostn,ptindex,ncols,n_Result) 
    ...
    CALL count_data_cols(funit,infile,nskip,ncols,n_Result)
    ...
    CALL find_reorder_index(infile,ns,geostn,ncols,stn,.TRUE.,ptindex,n_Result)
    IF(n_Result.NE.0) RETURN
  ...
  !-----------------------------------------------------------------------------------------
  SUBROUTINE find_reorder_index(file,n,oldstn,n2,newstn,allflag,aindex,n_Result) 
...
600 FORMAT(A32,I7,A100)
  ...
  !-------------------------------------------------------------------------------
  SUBROUTINE check_station(funit,infile,ns,nskip,geostn,nobsstn,oindex,n_Result) 
...
201 WRITE(6,*) 'Error: reading file',TRIM(infile)
  ...
  !-------------------------------------------------------------------------------
  SUBROUTINE check_q_stn(funit,infile,ncols,geostn,nobsstn,qindex,n_Result) 
...
201 WRITE(6,*) 'Error: reading file',TRIM(infile)
  ...
  !>and save that information. Used for Xobs.txt
  !-------------------------------------------------------------------------------
  SUBROUTINE check_xobs(funit,infile,ncols,nvars,outname,varinfo,n_Result) 

    !Argument declarations
    INTEGER, INTENT(IN)  :: funit                     !<File unit for file
    CHARACTER (LEN=*), INTENT(IN) :: infile           !<Name of file to be read
    INTEGER, INTENT(IN)  :: ncols                     !<Total number of columns
    INTEGER, INTENT(IN)  :: nvars                     !<maximum number of output variables
    CHARACTER (LEN=*), INTENT(IN)  :: outname(nvars)  !<outvar shortnames in MODEL
...
!    CHARACTER(LEN=4) varname(ncols)     !Short name of variable in Xobs.txt
    CHARACTER(LEN=4) lowname(nvars)     !Short name of variables in lower case
    CHARACTER(LEN=100) str
    INTEGER i,j,beg
    ...
    n_Result = 0
    lowname=outname
    CALL lower_case(lowname(1:nvars),nvars)
    OPEN(UNIT = funit,FILE = infile, STATUS = 'old',ACTION='read',ERR=200)
    READ(funit,*,ERR=201)                            !Skip one comment line

    !Read variables and subbasins in Xobs.txt
    READ(funit,*,ERR=201) str,varname(1:ncols)       !Read variable shortname
    CALL lower_case(varname(1:ncols),ncols)
    DO j=1,ncols
      beg = 1
      IF(varname(j)(1:2)=='rg'.OR.varname(j)(1:2)=='up') beg=3
      DO i=1,nvars
        IF(varname(j)(beg:beg+3)==lowname(i))THEN
          varnumber(j)=i                  !Set variable idnumber
          EXIT
        ENDIF
      ENDDO
    ENDDO
    READ(funit,*) str,daroid(1:ncols)        !Read variable subbasin id
...
!  !>Opens a file with observations, reads which variables it contains for which 
!  !>outregions and return that information. Used for Xoregobs.txt
!  !-------------------------------------------------------------------------------
!  SUBROUTINE check_xoregobs(funit,infile,ncols,nvars,outname,varinfo,n_Result) 
!
!    !Argument declarations
!    INTEGER, INTENT(IN)  :: funit                     !<File unit for file
!    CHARACTER (LEN=*), INTENT(IN) :: infile           !<Name of file to be read
...
!    INTEGER, INTENT(IN)  :: nvars                     !<maximum number of output variables
!    CHARACTER (LEN=*), INTENT(IN)  :: outname(nvars)  !<outvar shortnames in MODEL
!    INTEGER, INTENT(OUT) :: varinfo(ncols,2)          !<xobs variable numbers and outregionid
!    INTEGER, INTENT(OUT) :: n_Result                  !<error number
!    
!    !Local variables
!    CHARACTER(LEN=4) varname(ncols)     !Short name of variable in Xobs.txt
...
!    CHARACTER(LEN=100) str
!    INTEGER i,j
!    INTEGER varnumber(ncols)            !Outvarid number of variable for column in Xoregobs.txt
!    INTEGER daroid(ncols)               !Outregion identification number for corresponding column in Xoregobs.txt
!
!    !Start of subroutine
!    n_Result = 0
!    lowname=outname
...
!    READ(funit,*,ERR=201)                            !Skip one comment line
!
!    !Read variables and outregions in Xoregobs.txt
!    READ(funit,*,ERR=201) str,varname(1:ncols)       !Read variable shortname
!    CALL lower_case(varname(1:ncols),ncols)
!    DO j=1,ncols
!      DO i=1,nvars
!        IF(varname(j)==lowname(i))THEN
!          varnumber(j)=i                  !Set variable idnumber
!          EXIT
!        ENDIF
!      ENDDO
!    ENDDO
!    READ(funit,*) str,daroid(1:ncols)        !Read variable outregions id
!    
!    !Set output to variable names and subbasins
!    varinfo(:,1) = varnumber(:)
!    varinfo(:,2) = daroid(:)
...
!    RETURN
!
!  END SUBROUTINE check_xoregobs

  !>Opens a file with observations and reads to starting date
  !-------------------------------------------------------------------------------
  SUBROUTINE prepare_read_matrix(fileunit,infile,nskip,bdate,n_Result) 
...
611 WRITE(6,*) 'Error: reading file',TRIM(infile)
  ...
  SUBROUTINE check_obs_timeperiod(fileunit,infile,nskip,bdate,edate,&
                filestartdate,fileenddate,notimefound,n_Result) 
...
903 WRITE(6,*) 'ERROR: reading file '//TRIM(infile)
  ...
  SUBROUTINE write_integer_header(funit,n,columnid,firstcolumn)
    ...
    CHARACTER(LEN=*) :: firstcolumn    !<first column header
  ...
  SUBROUTINE write_mathsep(iout,n,x,nsig,textin,separator)
    ...
    CHARACTER (LEN=16)    t
    CHARACTER (LEN=1700000) outtxt           !(16+1)*100000
  ...
  !-------------------------------------------------------------------------------
  SUBROUTINE count_data_cols(funit,infile,nskip, ncols,n_Result)
...
612 WRITE(6,*) 'ERROR reading lines to skip in file: ', TRIM(infile)
  ...
  !---------------------------------------------------------------------------------------------
  SUBROUTINE count_data_rows(funit,infile,nskip,n,n_Result) 
...
301 CONTINUE
  ...
  !>\brief Reads a matrix of identification combinations for classes
  !---------------------------------------------------------------------------------------------
  SUBROUTINE read_geoid(funit,infile,n,maxx,x,dmax) 

    !Argument declarations
    INTEGER, INTENT(IN)  :: funit           !<Unit for file
    CHARACTER (LEN=*), INTENT(IN) :: infile !<Name of file to be read
    INTEGER, INTENT(IN)  :: n               !<Maximum number of identifications
    INTEGER, INTENT(IN)  :: maxx            !<Maximum number of identification combinations
    REAL, INTENT(OUT) :: x(n,maxx)          !<Identification numbers
...
4000 str = ''      !empty line
  ...
  !>\brief Read next date as string from line, ignoring beginning
  !>blanks and tabs
  !!Supports formats yyyy-mm-dd, yyyy-mm-dd hh:mm, yyyymmdd and
  !!yyyymmddhhmm
  !------------------------------------------------------------------------------------------
  SUBROUTINE read_next_date_on_line(linelen,strlen,pos,line,str,nostrfound,notimefound,errstr) 
    ...
    str(j:j+9) = line(i:i+9)          ! TODO: check content!
    IF(str(5:5)=='-')THEN
      !Normal date
      i = i + 10
      j = j + 10
