...
MODULE SOIL_PROCESSES
  ...
  USE STATETYPE_MODULE, ONLY :soilstatetype
...
CONTAINS
  ...
  SUBROUTINE initiate_soil_water_state(soilstate)
    ...
    DO j = 1,nclass
      ...
      ENDIF
    ENDDO
    !>Initiate soil to saturation (fc+wp+ep)
    DO i = 1,nsub
  ...
  SUBROUTINE calculate_snow(i,j,subid,iluse,snowfall,csnowfall,snow,csnow,temp,melt, &
    ...
    !>Calculate evaporation from snow (sublimation), with/without snowcover scaling
    IF(modeloption(p_snowevap).GE.1)THEN
      IF(snow>0)THEN
        evap = epot * effcov
        !Calculate ablation = melt + sublimation and limit it to available snow pack
        ...
        IF(abla<abla0)THEN
          ...
          IF(melt+evap>snow) evap = snow - melt !safe?
        ENDIF      
      ELSE
    ...
    ELSE
      ...
      melt = MAX(0.,MIN(melt,snow))   !check melt agains snow pack
    ENDIF
...
!    epotcorr = 1 + genpar(m_cevpam)*SIN(2.*pi*(dayno-genpar(m_cevpph))/365.) 
    ...
    IF(modeloption(p_petmodel).GT.1)THEN
      ...
      kc = landpar(m_kc,classdata(j)%luse)
      !Turbidity
  ...
  SUBROUTINE calculate_actual_soil_evapotranspiration(i,j,temp,epot,wp,fc,  &
    ...
    IF(temp>tt)THEN
      ...
      IF(soilstate%water(1,j,i) - wp(1)> basinlp(i) * fc(1)) THEN
        evap1 = epot*epotfrac(1)
      ELSEIF(soilstate%water(1,j,i)-wp(1) <= 0.0) THEN
        evap1 = 0.0
      ELSE
        evap1 = epot*epotfrac(1)*((soilstate%water(1,j,i)-wp(1))/(basinlp(i) * fc(1)))
      ENDIF
      ...
      IF(evap1<soilstate%water(1,j,i))THEN
      ...
      ELSE
        ...
        soilstate%conc(:,1,j,i) = 0.    !remove last traces, safe for wp=0
      ENDIF
      ...
      IF(soilthick(2,j)>0)THEN
        ...
        IF(soilstate%water(2,j,i)-wp(2) > basinlp(i) * fc(2)) THEN
          evap2 = epot*epotfrac(2)
        ELSEIF(soilstate%water(2,j,i)-wp(2) <= 0.0) THEN
          evap2 = 0.0
        ELSE
          evap2 = epot*epotfrac(2)*((soilstate%water(2,j,i)-wp(2))/(basinlp(i) * fc(2)))
        ENDIF
        ...
        IF(evap2<soilstate%water(2,j,i))THEN
        ...
        ELSE
          ...
          soilstate%conc(:,2,j,i) = 0.  !remove last traces, safe for wp=0
        ENDIF
      ELSE
        evap2 = 0.
        cevap2 = 0.
...
!                       streamdepth
  ...
  !-----------------------------------------------------------------------------------
  SUBROUTINE infiltration(i,j,isoil,wp,fc,ginfilt,cginfilt,infilt,  &
