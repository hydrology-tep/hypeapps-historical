...
!                      dayno, &  !Use pseudo_dayno as argument instead
                      ...
                      load,vegtype,basin,classbasin,path, &
                      glacier,glacierindex,nglaciers,  &
                      numsubstances,nsub,maxsoillayers, &
                      tiledepth,soilthick,soildepth,streamdepth, &
  ...
  USE HYPEVARIABLES, ONLY : wpmm,fcmm,epmm, &
                            ...
                            m_degradhp,m_denitrlu,m_hsatINsoil, &
                            ...
                            m_glacttmp,m_glaccmlt,m_glaccmrad,m_glacalb,m_glaccmrefr,m_fepotglac
...
CONTAINS
  ...
  !--------------------------------------------------------------
  SUBROUTINE soilmodel_3(i,j,isoil,iluse,subid,classarea,prec,cprec,temp, & 
       daylength,cevpcorr,rrcscorr,phoscorr,dayno,  &
       ...
       glac_part,nonglac_part,snowfall,rainfall,cropsources,ruralaload,rgrwload,atmdepload,sffrac,swrad,  &
       radext,netrad,actvap,satvap,wind,infiltrationflows,glacierflows, &
    ...
    REAL, INTENT(OUT) :: evapsnow  !<evaporation from snow covered land
    ...
    CALL add_dry_deposition_to_landclass(i,j,iluse,conductN,conductP,classarea,atmdepload,vegtype(j),load(i)%indrydep,     &
    ...
    !Calculate degree of snow cover on snow plain
!    CALL calculate_fractional_snowcover(iluse,0.,frozenstate%snow(j,i),frozenstate%snowmax(j,i),frozenstate%snowcov(j,i))
!    CALL calculate_snowdepth(frozenstate%snow(j,i),oldsnow,genpar(m_sndens0),genpar(m_dsndens),frozenstate%snowage(j,i),frozenstate%snowdepth(j,i))
    ...
    CALL calculate_soil_runoff(i,j,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),soildepth(:,j),soilthick(:,j),streamdepth(j),soilstate,soilrunoff,csoilrunoff)
    ...
    IF(surfaceflow > 0.) THEN
       ...
       !Temperature content in surface flow [DG/JS Temp.model May2013]
       IF(i_t2>0) csrunoff(i_t2) = (cinfilt(i_t2) * satoverflow + excessinfilt * cexcessinfilt(i_t2)) / (satoverflow + excessinfilt)    
        !JS4 (här borde kanske marktemperaturen in i stället för cinfilt(i_t2)
    ENDIF

    !Erosion of particulate phosphorus with fastflow (surface flow and macropore flow) including delay in temporary storage.
    !The eroded PP is given to all runoff types 
    ...
    !Groundwater level
    CALL calculate_groundwater_table(soilstate%water(:,j,i),wpmm(:,j),fcmm(:,j),epmm(:,j),pwmm,soildepth(:,j),soilthick(:,j),gwat) 
    ...
    CALL soil_np_processes(i,j,iluse,conductN,conductP,conductC,dayno,classarea,wpmm(:,j),fcmm(:,j),epmm(:,j),plantuptake,soilthick(:,j),genpar(m_fertdays),genpar(m_littdays),  &
         ...
         landpar(m_denitrlu,iluse),genpar(m_hsatINsoil),landpar(m_degradhp,iluse),soilstate)
    ...
    IF(i_t1>0)THEN
      IF(T1release>0.)THEN
        IF(surfaceflow>0.)THEN
          csrunoff(i_t1) = csrunoff(i_t1) + (T1release * (surfaceflow/(infilt+surfaceflow))) / surfaceflow
        ENDIF
        soilstate%partT1(1,j,i) = soilstate%partT1(1,j,i) + T1release * (infilt/(infilt+surfaceflow))
      ENDIF
    ENDIF      
  ...
  SUBROUTINE get_glacier_parameters(nc,arr)
    ...
    IF(genpar(m_glacdens).LE.0.)   genpar(m_glacdens)   = 0.85
