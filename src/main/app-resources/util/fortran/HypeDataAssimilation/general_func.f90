...
CONTAINS
  ...
  !>
  !>\b Reference ModelDescription Chapter 5 (Sedimentation)
  !-----------------------------------------------------------------
  SUBROUTINE sedimentation(vol,conc,vel,area)

    !Argument declarations
    REAL, INTENT(IN)              :: vol        !<water volume (m3)
    ...
    REAL, INTENT(IN)              :: area       !<bottom area (m2)

    !Local variables
    REAL pool      !pool in water (g or other unit)
    REAL sedloss   !change (g/timestep)

    !Calculate pool
    pool = vol * conc

    !Calculate sedimentation
    sedloss = vel * area * conc
