...
!> Contais module random_routines.

!> Module with routines for random number generation, including routines for spatially correlated random fields.
...
!>TYPE randxy_data: data needed to generate spatially correlated random data for a vector of X,Y points
  TYPE randxy_data
    ...
    real, allocatable :: pert_field(:,:) !> spatially correlated 2D field (nxl,nyl)
    real              :: gridsize        !> grid size in x and y direction
    integer           :: nxl,nyl         !> grid dimensions
    real, allocatable :: xfield(:),yfield(:) !> vector with x and y coordinates in the 2D field
    
    ! variables for the 2D spectral correlation field
    real, allocatable :: corr(:,:)         ! dimensions (nxl,nyl)
    real, allocatable :: spec_dens_2D(:,:) ! dimensions (0:kmax,-lmax:lmax)
    integer           :: kmax,lmax         ! dimensions (kmax=nxl/2,lmax=nyl/2)

    ! parameters defining the spatial correlation
    real              :: lscale        !> corr.length
    integer           :: index_corr    !> corr.type: 1 Gaussian, 2 Compact 5th degree polynomial, 3 Power law
...
! statistics:               mean, variance, standard deviation, covariance, etc of vectors and matrices
! maths:                    vector and matrix multiplication and inversion, etc
...
!! @param[in] umin    minimum value
!! @param[in] umax    maximum value
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
!> Function rgauss
!! generation pseudo-random number from normal distribution N(0,1)
...
!-----------------------------------------------------------------------------------------
  ...
  END FUNCTION rgauss
!------------------------------------------------------------------------------------------
!> Subroutine resample_randxy_data
!!
!! pseudo-random number (gaussian) with spatial correlation for point coordinates (x,y)
!!
...
!------------------------------------------------------------------------------------------
  SUBROUTINE resample_randxy_data(randxy)
    ...
    ! 1) generate a new 2D random field
    ...
    ! 2) interpolate to the x,y coordinates
...
!------------------------------------------------------------------------------------------
  SUBROUTINE init_randxy_data(randxy,np,xp,yp,lscale,gridsize,index_corr)
    ...
    REAL, INTENT(IN)          :: xp(np), yp(np)!> easting(x) and northing(y) coordinates
    ...
    INTEGER, INTENT(IN)       :: index_corr    !> corr.type: 1 Gaussian, 2 Compact 5th degree polynomial, 3 Power law
    ...
    !Secondly, make sue we fulfil the fft991 requirements
...
!------------------------------------------------------------------------------
!> Subroutine GEN_CORR: Generate correlation function in physical space
!------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
! BLOCKSTART - SPATIAL INTEROLATION
!
!-----------------------------------------------------------------------------------------
!>SUBROUTINE interpol_grid2point
!! interpolation from regular grid Z(X,Y) to points zp(xp,yp)
...
!---
! BLOCKEND - SPATIAL INTERPOLATION
!-----------------------------------------------------------------------------------------
