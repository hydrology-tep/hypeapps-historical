...
CONTAINS
  ...
  !-----------------------------------------------------------------
  SUBROUTINE initiate_river_npc_state(initN,initP,initC,initT,riverstate)
    ...
    IF(initT.AND.i_t2>0) riverstate%cqueue(i_t2,:,:,:) = genpar(m_iniT2)
  ...
  !-----------------------------------------------------------------
  SUBROUTINE initiate_lake_npc_state(initN,initP,initC,initT,initilake,initolake,lakestate)
    ...
    IF(initN.OR.initP.OR.initC.OR.initT)THEN
      CALL set_lake_slowwater_maxvolume(nsub,genpar(m_gldepi),basin(:)%lakedepth, &
  ...
  SUBROUTINE set_lake_slowwater_maxvolume(n,ildepth,oldepth,ldparindex,ldpar,initilake,initolake)
    ...
    REAL, INTENT(IN)    :: oldepth(n) !<olake depth (basin(:)%lake_depth)
  ...
  SUBROUTINE add_dry_deposition_to_river(i,areaij,pooltype,source,veg,dryin,drypp,riverstate)
    ...
    !Prepare dry deposition
    sourcedd = 0.
    IF(i_in>0 .AND. veg>0) sourcedd(i_in) = dryin(veg)*areaij   !kg/km2*m2 -> mg (=m3*ug/L)
