...
CONTAINS
  ...
  SUBROUTINE initiate_soil_npc_state(initN,initP,initC,maxlayers,soilstate)
    ...
    !Local variables
    INTEGER i,j           !loop-variables (subbasin,class,soillayer)
    ...
    IF(initP)THEN
      DO i = 1,nsub
        IF(basin(i)%wqparregion>0)THEN
          phoscorr = 1. + wqregpar(m_phoscorr,basin(i)%wqparregion)   !Regional correction of phosphorus level
        ELSE
          phoscorr  = 1.
        ENDIF
        DO j = 1,nclass   
  ...
  SUBROUTINE calculate_plant(i,j,dayno,temp,daylength,common_uptake,miscstate)
    ...
    REAL, INTENT(OUT)   :: common_uptake(2,2)       !<plant uptake (kg/km2), (N:P,sl1:sl2)
    ...
    DO kcrop = 1, 2                             !main and secondary crop
      ...
      IF(part>0)THEN
        ...
        IF(modeloption(p_growthstart)==1)THEN
          IF(dayno == 1) THEN            !reset (this will not work on the southern hemisphere?)
  ...
  SUBROUTINE crop_sources(i,j,dayno,calcN,calcP,calcC,ndays,ldays,areaij,thickness,soilstate,sources)
    ...
    REAL common_nadd(maxsoillayers,2)                  !kg/km2
    REAL common_padd(maxsoillayers,2)                  !kg/km2
    REAL common_nres(maxsoillayers,2)                  !kg/km2
    REAL common_pres(maxsoillayers,2)                  !kg/km2          
    REAL common_cres(maxsoillayers,2)                  !kg/km2          
    REAL part                           !part of class area that is the current crop
    REAL littdays
...
!    areaij=classbasin(i,j)%part*basin(i)%area*1.E-6   !(km2)
    common_nadd   = 0.
    common_padd   = 0.
    common_nres   = 0.
    common_pres   = 0.
      ...
      IF(part>0)THEN
        ...
        IF((dayno >= cropdata(k)%fertday1 .AND. dayno < cropdata(k)%fertday1 + fertperiod) .OR. &
          ...
          IF(thickness(2)>0)THEN
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%fertnamount1 * (1. - cropdata(k)%fertdown1) / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%fertpamount1 * (1. - cropdata(k)%fertdown1) / ndays
            common_nadd(2,1) = common_nadd(2,1) + part * cropdata(k)%fertnamount1 * cropdata(k)%fertdown1 / ndays
            common_padd(2,1) = common_padd(2,1) + part * cropdata(k)%fertpamount1 * cropdata(k)%fertdown1 / ndays
          ELSE
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%fertnamount1 / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%fertpamount1 / ndays
          ENDIF
        ENDIF
        IF((dayno >= cropdata(k)%fertday2 .AND. dayno < cropdata(k)%fertday2 + fertperiod) .OR. &
          ...
          IF(thickness(2)>0)THEN
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%fertnamount2 * (1. - cropdata(k)%fertdown2) / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%fertpamount2 * (1. - cropdata(k)%fertdown2) / ndays
            common_nadd(2,1) = common_nadd(2,1) + part * cropdata(k)%fertnamount2 * cropdata(k)%fertdown2 / ndays
            common_padd(2,1) = common_padd(2,1) + part * cropdata(k)%fertpamount2 * cropdata(k)%fertdown2 / ndays
          ELSE
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%fertnamount2 / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%fertpamount2 / ndays
          ENDIF
        ENDIF
        IF((dayno >= cropdata(k)%manday1 .AND. dayno < cropdata(k)%manday1 + fertperiod) .OR.  &
             (dayno < cropdata(k)%manday1 + fertperiod - prevdoy))THEN
          IF(thickness(2)>0)THEN
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%mannamount1 * (1. - cropdata(k)%mandown1) * inorgpart / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%manpamount1 * (1. - cropdata(k)%mandown1) * fastppart / ndays
            common_nadd(1,2) = common_nadd(1,2) + part * cropdata(k)%mannamount1 * (1. - cropdata(k)%mandown1) * (1.-inorgpart) / ndays
            common_padd(1,2) = common_padd(1,2) + part * cropdata(k)%manpamount1 * (1. - cropdata(k)%mandown1) * (1.-fastppart) / ndays
            common_nadd(2,1) = common_nadd(2,1) + part * cropdata(k)%mannamount1 * cropdata(k)%mandown1 * inorgpart / ndays
            common_padd(2,1) = common_padd(2,1) + part * cropdata(k)%manpamount1 * cropdata(k)%mandown1 * fastppart / ndays
            common_nadd(2,2) = common_nadd(2,2) + part * cropdata(k)%mannamount1 * cropdata(k)%mandown1 * (1.-inorgpart) / ndays
            common_padd(2,2) = common_padd(2,2) + part * cropdata(k)%manpamount1 * cropdata(k)%mandown1 * (1.-fastppart) / ndays
          ELSE
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%mannamount1 * inorgpart / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%manpamount1 * fastppart / ndays
            common_nadd(1,2) = common_nadd(1,2) + part * cropdata(k)%mannamount1 * (1.-inorgpart) / ndays
            common_padd(1,2) = common_padd(1,2) + part * cropdata(k)%manpamount1 * (1.-fastppart) / ndays
          ENDIF
        ENDIF
        IF((dayno >= cropdata(k)%manday2 .AND. dayno < cropdata(k)%manday2 + fertperiod) .OR.  &
             (dayno < cropdata(k)%manday2 + fertperiod - prevdoy))THEN
          IF(thickness(2)>0)THEN
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%mannamount2 * (1. - cropdata(k)%mandown2) * inorgpart / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%manpamount2 * (1. - cropdata(k)%mandown2) * fastppart / ndays
            common_nadd(1,2) = common_nadd(1,2) + part * cropdata(k)%mannamount2 * (1. - cropdata(k)%mandown2) * (1.-inorgpart) / ndays
            common_padd(1,2) = common_padd(1,2) + part * cropdata(k)%manpamount2 * (1. - cropdata(k)%mandown2) * (1.-fastppart) / ndays
            common_nadd(2,1) = common_nadd(2,1) + part * cropdata(k)%mannamount2 * cropdata(k)%mandown2 * inorgpart / ndays
            common_padd(2,1) = common_padd(2,1) + part * cropdata(k)%manpamount2 * cropdata(k)%mandown2 * fastppart / ndays
            common_nadd(2,2) = common_nadd(2,2) + part * cropdata(k)%mannamount2 * cropdata(k)%mandown2 * (1.-inorgpart) / ndays
            common_padd(2,2) = common_padd(2,2) + part * cropdata(k)%manpamount2 * cropdata(k)%mandown2 * (1.-fastppart) / ndays
          ELSE
            common_nadd(1,1) = common_nadd(1,1) + part * cropdata(k)%mannamount2 * inorgpart / ndays
            common_padd(1,1) = common_padd(1,1) + part * cropdata(k)%manpamount2 * fastppart / ndays
            common_nadd(1,2) = common_nadd(1,2) + part * cropdata(k)%mannamount2 * (1.-inorgpart) / ndays
            common_padd(1,2) = common_padd(1,2) + part * cropdata(k)%manpamount2 * (1.-fastppart) / ndays
          ENDIF
        ENDIF
         !Calculate common residues
        IF(cropdata(k)%resdayno==0) littdays = 365.   !litterfall every day  !CP121031
        IF(cropdata(k)%resdayno==0 .OR.   &
          ...
          IF(thickness(2)>0)THEN
            common_nres(1,1) = common_nres(1,1) + cropdata(k)%resfast * part * cropdata(k)%resnamount * (1. - cropdata(k)%resdown) / littdays
            common_nres(1,2) = common_nres(1,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%resnamount * (1. - cropdata(k)%resdown) / littdays
            common_pres(1,1) = common_pres(1,1) + cropdata(k)%resfast * part * cropdata(k)%respamount * (1. - cropdata(k)%resdown) / littdays
            common_pres(1,2) = common_pres(1,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%respamount * (1. - cropdata(k)%resdown) / littdays
            common_nres(2,1) = common_nres(2,1) + cropdata(k)%resfast * part * cropdata(k)%resnamount * cropdata(k)%resdown / littdays
            common_nres(2,2) = common_nres(2,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%resnamount * cropdata(k)%resdown / littdays
            common_pres(2,1) = common_pres(2,1) + cropdata(k)%resfast * part * cropdata(k)%respamount * cropdata(k)%resdown / littdays  
            common_pres(2,2) = common_pres(2,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%respamount * cropdata(k)%resdown / littdays
            common_cres(1,1) = common_cres(1,1) + part * cropdata(k)%rescamount * (1. - cropdata(k)%resdown) / littdays  !CP121031
            common_cres(2,1) = common_cres(2,1) + part * cropdata(k)%rescamount * cropdata(k)%resdown / littdays
          ELSE
            common_nres(1,1) = common_nres(1,1) + cropdata(k)%resfast * part * cropdata(k)%resnamount / littdays
            common_nres(1,2) = common_nres(1,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%resnamount / littdays
            common_pres(1,1) = common_pres(1,1) + cropdata(k)%resfast * part * cropdata(k)%respamount / littdays
            common_pres(1,2) = common_pres(1,2) + (1. - cropdata(k)%resfast) * part * cropdata(k)%respamount / littdays
            common_cres(1,1) = common_cres(1,1) + part * cropdata(k)%rescamount / littdays 
          ENDIF
        ENDIF
      ENDIF
    ENDDO
    ...
    IF(calcN .AND. SUM(common_nadd(:,1))>0.)THEN
      IF(soilstate%water(1,j,i)>0.)THEN
        CALL add_source_to_water(soilstate%water(1,j,i),1,soilstate%conc(i_in,1,j,i),common_nadd(1,1))
        sources(1,i_in) = sources(1,i_in) + common_nadd(1,1)*areaij  !InorgN load from fertilizer from soil layer 1 (kg)
      ELSE
        CALL production_pool(1,soilstate%fastN(1,j,i),common_nadd(1,1))
        sources(1,i_on) = sources(1,i_on) + common_nadd(1,1)*areaij  !OrgN load from fertilizer from soil layer 1 (kg)
      ENDIF
      IF(soilstate%water(2,j,i)>0.)THEN
        CALL add_source_to_water(soilstate%water(2,j,i),1,soilstate%conc(i_in,2,j,i),common_nadd(2,1))
        sources(1,i_in) = sources(1,i_in) + common_nadd(2,1)*areaij  !InorgN load from fertilizer from soil layer 2 (kg)
      ELSEIF(thickness(2)>0)THEN
        CALL production_pool(1,soilstate%fastN(2,j,i),common_nadd(2,1))
        sources(1,i_on) = sources(1,i_on) + common_nadd(2,1)*areaij  !OrgN load from fertilizer from soil layer 2 (kg)
      ENDIF
    ENDIF
    IF(calcP .AND. SUM(common_padd(:,1))>0.)THEN
      IF(soilstate%water(1,j,i)>0.)THEN
        CALL add_source_to_water(soilstate%water(1,j,i),1,soilstate%conc(i_sp,1,j,i),common_padd(1,1))
        sources(1,i_sp) = sources(1,i_sp) + common_padd(1,1)*areaij        !SRP load from fertilizer, soil layer 1 (kg)
      ELSE
        CALL production_pool(1,soilstate%fastP(1,j,i),common_padd(1,1))
        sources(1,i_pp) = sources(1,i_pp) + common_padd(1,1)*areaij        !PartP load from fertilizer, soil layer 1 (kg)
      ENDIF
      IF(soilstate%water(2,j,i)>0.)THEN
        CALL add_source_to_water(soilstate%water(2,j,i),1,soilstate%conc(i_sp,2,j,i),common_padd(2,1))
        sources(1,i_sp) = sources(1,i_sp) + common_padd(2,1)*areaij        !SRP load from fertilizer, soil layer 2 (kg)
      ELSEIF(thickness(2)>0)THEN
        CALL production_pool(1,soilstate%fastP(2,j,i),common_padd(2,1))
        sources(1,i_pp) = sources(1,i_pp) + common_padd(2,1)*areaij        !PartP load from fertilizer, soil layer 2 (kg)
      ENDIF
    ENDIF
    IF(calcN .AND. SUM(common_nadd(:,2))>0.)THEN
      IF(thickness(2)>0)THEN
        CALL production_pool(maxsoillayers,soilstate%fastN(:,j,i),common_nadd(:,2))
        sources(1,i_on) = sources(1,i_on) + SUM(common_nadd(:,2))*areaij  !OrgN load from fertilizer, both soil layers (kg)
      ELSE
        CALL production_pool(1,soilstate%fastN(1,j,i),common_nadd(1,2))
        sources(1,i_on) = sources(1,i_on) + common_nadd(1,2)*areaij       !OrgN load from fertilizer, soil layer 1 (kg)
      ENDIF
    ENDIF
    IF(calcP .AND. SUM(common_padd(:,2))>0.)THEN
      IF(thickness(2)>0)THEN
        CALL production_pool(maxsoillayers,soilstate%fastP(:,j,i),common_padd(:,2))
        sources(1,i_pp) = sources(1,i_pp) + SUM(common_padd(:,2))*areaij  !PartP load from fertilizer, both soil layers (kg)
      ELSE
        CALL production_pool(1,soilstate%fastP(1,j,i),common_padd(1,2))
        sources(1,i_pp) = sources(1,i_pp) + common_padd(1,2)*areaij        !PartP load from fertilizer, soil layer 1 (kg)
      ENDIF
    ENDIF

    !Add residues
    IF(calcN .AND. SUM(common_nres(:,:))>0.)THEN
      IF(thickness(2)>0)THEN
        CALL production_pool(maxsoillayers,soilstate%fastN(:,j,i),common_nres(:,1))
        CALL production_pool(maxsoillayers,soilstate%humusN(:,j,i),common_nres(:,2))
      ELSE
        CALL production_pool(1,soilstate%fastN(1,j,i),common_nres(1,1))
        CALL production_pool(1,soilstate%humusN(1,j,i),common_nres(1,2))
      ENDIF
      sources(2,i_on) = SUM(common_nres(:,:))*areaij                !OrgN load from residuals, all soil layers and N types (kg)
    ENDIF
    IF(calcP .AND. SUM(common_pres(:,:))>0.)THEN                
      IF(thickness(2)>0)THEN
        CALL production_pool(maxsoillayers,soilstate%fastP(:,j,i),common_pres(:,1))     
        CALL production_pool(maxsoillayers,soilstate%humusP(:,j,i),common_pres(:,2))    
      ELSE
        CALL production_pool(1,soilstate%fastP(1,j,i),common_pres(1,1))                
        CALL production_pool(1,soilstate%humusP(1,j,i),common_pres(1,2))                
      ENDIF
      sources(2,i_pp) = SUM(common_pres(:,:))*areaij                !PartP load from residuals, all soil layers and P types (kg)
    ENDIF
    IF(calcC .AND. SUM(common_cres(:,:))>0.)THEN                
      IF(thickness(2)>0)THEN
        CALL production_pool(maxsoillayers,soilstate%fastC(:,j,i),common_cres(:,1))     
      ELSE
        CALL production_pool(1,soilstate%fastC(1,j,i),common_cres(1,1))                
      ENDIF
      sources(2,i_oc) = SUM(common_cres(:,:))*areaij                !OrgC load from residuals, all soil layers (kg)
    ENDIF

  END SUBROUTINE crop_sources

  !>\brief Erosion of soil phosphorus with fast flow, surface flow and 
  ...
  !---------------------------------------------------------------------------------------
  SUBROUTINE runoff_pp_by_erosion(i,j,isoil,iluse,dayno,prec,surfacerunoff,macroporflow,    &
                          tilerunoff,totflow,phoscorr,csurface,cmacro,ctile,csoil1,csoil2,  &
                          csoil3,snow,soilstate)
    ...
    REAL, INTENT(INOUT) :: csurface    !<PP concentration surface runoff (mg/L)
    REAL, INTENT(INOUT) :: cmacro      !<PP concentration macropores (mg/L)
    REAL, INTENT(INOUT) :: ctile       !<PP concentration tile drainage (mg/L)
    REAL, INTENT(INOUT) :: csoil1      !<PP concentration soil runoff layer 1 (mg/L)
    REAL, INTENT(INOUT) :: csoil2      !<PP concentration soil runoff layer 2 (mg/L)
    REAL, INTENT(INOUT) :: csoil3      !<PP concentration soil runoff layer 3 (mg/L)
    REAL,INTENT(IN)     :: snow        !<snow water (mm)
    TYPE(soilstatetype),INTENT(INOUT)  :: soilstate   !<Soil states

    !Local variables
    REAL PPrel                !PP released from delay pool
    ...
    REAL macroppconc,surfrppconc  !variables to temporary hold PP-konc of macropores and surfacerunoff 
    REAL newppconc                !PP concentration for PP released from delay pool

    !>\b Algorithm \n
    !>Calculate PP erosion
    macroppconc = 0; surfrppconc = 0.
    CALL calculate_erosion(i,j,dayno,prec,soilthick(1,j),surfacerunoff, &   !calculate mobilised PP at soil surface
         erodedP,soilpar(m_soilcoh,isoil),phoscorr*soilpar(m_soilerod,isoil), &
         snow,genpar(m_sreroexp),soilstate%partP(:,j,i),   &
         soilstate%humusP(:,j,i),fracminP)
    erodingflow = surfacerunoff + macroporflow
    CALL calculate_transport(erodingflow,erodedP)   !calculate amount of PP transported in fast flow paths  (kg/km2)
    
    IF(surfacerunoff > 0.) THEN     !erodedP goes back to soil if no surface runoff
      !>Calculate resulting PP concentration of surface runoff and macroporeflow
      ...
      IF(tilerunoff > 0.) THEN       !only when runoffd>0  will PP from macropor reach runoff
        IF(macroporflow > 0. )THEN
          macroppconc =  soilpar(m_macfilt,isoil) * erodedP / erodingflow
        ENDIF
      ENDIF
    ...
    !>Add PP of macropore flow to temporary PP pool 
    IF(macroporflow > 0.) THEN
      macroppconc = macroppconc + cmacro    
      cmacro = 0.                          
    ...
    IF(totflow > 0.) THEN
      ...
      soilstate%PPrelpool(j,i) = soilstate%PPrelpool(j,i) + ctile * tilerunoff + csurface * surfacerunoff   !add sources
      ...
      csurface = newppconc
      ctile = newppconc
      csoil1 = csoil1 + newppconc
      csoil2 = csoil2 + newppconc
      csoil3 = csoil3 + newppconc
    ENDIF

  END SUBROUTINE runoff_pp_by_erosion
  ...
  !------------------------------------------------------------------
  SUBROUTINE calculate_erosion(i,j,dayno,prec,thickness,surfacerunoff,erodedP,  &
       cohesion,erodibility,snow,sreroexp,partP,humusP,fracminP)     
    ...
    INTEGER, INTENT(IN)   :: i             !<index of current subbasin
    INTEGER, INTENT(IN)   :: j             !<index of current class
    INTEGER, INTENT(IN)   :: dayno         !<current day number of the year
    REAL, INTENT(IN)      :: prec          !<precipitation (rainfall only)    
    ...
    REAL, INTENT(OUT)     :: erodedP       !<eroded phosphorus (kg/km2)
    REAL, INTENT(IN)      :: cohesion      !<(kPa)
    REAL, INTENT(IN)      :: erodibility   !<(g/J)
    REAL, INTENT(IN)      :: snow          !<snow water (mm)
    REAL, INTENT(IN)      :: sreroexp      !<surface runoff erosion exponent 
    REAL, INTENT(IN)      :: partP(maxsoillayers)      !<partP pool (kg/km2)
    REAL, INTENT(IN)      :: humusP(maxsoillayers)     !<humusP pool (kg/km2)
    REAL, INTENT(OUT)     :: fracminP      !<part of eroded P in mineral form   

    !Local variables
    INTEGER k,kcrop
    REAL MobilisedSed, MobilisedP, Rainfall_energy, cropcover, groundcover
    REAL intensity
    REAL part
    ...
    IF(prec > 0.) THEN
      IF(intensity > 0.) THEN
        IF(prec>5.0) THEN     !TODO: shorter timestep, other threshold?
        ...
        MobilisedP = 1.0E-3 * MobilisedSed * ((partP(1) + humusP(1)) / (thickness * bulkdensity))    !kgP / km2     
      ENDIF
    ENDIF
    IF(surfacerunoff > 0.) THEN   
       MobilisedSed = (((surfacerunoff * 365.) ** sreroexp) * (1. - common_groundcover) * (1./(0.5 * cohesion)) * sin(basin(i)%slope / 100.)) / 365. !g/m2   
       MobilisedP = MobilisedP + 1.0E-3 * MobilisedSed * ((partP(1) + humusP(1)) / (thickness * bulkdensity))         
    ENDIF

    !Set output variables
    erodedP = MobilisedP      !kg/km2
    fracminP = partP(1) / (partP(1) + humusP(1))  
  ...
  !! and soil surface processes - Erosion calculations)
  !------------------------------------------------------------------
  SUBROUTINE  calculate_transport(flow,erodedP)

    !Argument declarations
    REAL, INTENT(IN)    :: flow      !<Fast flow
    REAL, INTENT(INOUT) :: erodedP   !<Mobilised P
  ...
  !------------------------------------------------------------------------------
  SUBROUTINE soil_np_processes(i,j,calcN,calcP,calcC,dayno,area,wp,fc,ep, &
                      common_uptake,thickness,ndays,ldays,source,sink,nitrification,  &
                      denitrification,cropuptake,cropsources,pardisfN,pardisfP, &
                      pardishN,pardishP,parminfN,parminfP,pardegrhN,denpar,pardegrhP,soilstate)       
    ...
    IF(calcN)THEN
      denitrification = 0.
      IF(thickness(1)>0.) CALL soil_denitrification(wp(1)+fc(1)+ep(1),denpar,soilstate%water(1,j,i),soilstate%temp(1,j,i),soilstate%conc(:,1,j,i),sink)
      denitrification(1) = sink(i_in)
      IF(thickness(2)>0.) CALL soil_denitrification(wp(2)+fc(2)+ep(2),denpar,soilstate%water(2,j,i),soilstate%temp(2,j,i),soilstate%conc(:,2,j,i),sink)
      denitrification(2) = sink(i_in)
      IF(thickness(3)>0.) CALL soil_denitrification(wp(3)+fc(3)+ep(3),denpar,soilstate%water(3,j,i),soilstate%temp(3,j,i),soilstate%conc(:,3,j,i),sink)
  ...
  !----------------------------------------------------------------------
  SUBROUTINE soil_denitrification(maxwc,denpar,soil,stemp,csoil,sink)
    ...
    concfcn = halfsatconcfactor(csoil(i_in),halfsatINsoil)
...
!        IF(ABS(dx)/xn<0.000001) EXIT
  ...
  SUBROUTINE local_diffuse_source(i,j,pw,classarea,soilstate,ruralaload,upwardflow,ruralflow,cruralflow)
    ...
    USE MODVAR, ONLY : genpar,  &
                       ...
                       i_in,i_on,i_sp,i_pp,   &
                       soilthick
    USE HYPEVARIABLES, ONLY : landarea,   &
                              m_locsoil
    ...
    IF(genpar(m_locsoil)>0)THEN
      ...
      IF(qin>0)THEN
        ...
        IF(i_in>0)THEN 
          cin(i_in) = load(i)%tnconcloc * load(i)%inpartloc
          cin(i_on) = load(i)%tnconcloc * (1. - load(i)%inpartloc)
          ruralaload(i_in) = cin(i_in) * qin
          ruralaload(i_on) = cin(i_on) * qin
        ENDIF
        IF(i_sp>0)THEN 
          cin(i_sp) = load(i)%tpconcloc * load(i)%sppartloc
          cin(i_pp) = 0.
          ruralaload(i_sp) = cin(i_sp) * qin
        ENDIF
        cruralflow = cin
        IF(i_pp>0) cruralflow(i_pp) = load(i)%tpconcloc * (1. - load(i)%sppartloc)
        ...
        IF(i_sp>0)THEN 
          ppload(1) = load(i)%tpconcloc * (1. - load(i)%sppartloc) * qin       !kg/km2=mg/L*mm
