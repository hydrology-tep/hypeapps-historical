...
!> \{
  ...
  INTEGER, ALLOCATABLE, PUBLIC :: indx_par(:)     !<contains info on whether a given parameter is regionally estimated and which parameter order it has (max_par)
  INTEGER, ALLOCATABLE, PUBLIC :: ndes_reg(:,:)   !<number of descriptors used to regionalize each of the parameters in each group (ngroup_cat,npar_regr)
  INTEGER, ALLOCATABLE, PUBLIC :: idx_catdes(:,:,:)  !<indices of the pca of catchment descriptors used for regionalization of the parameter (ngroup_cat,npar_regr,npc_catdes)
  REAL, ALLOCATABLE, PUBLIC    :: wght_catdes(:,:,:) !<weights of the pca used to estimate the model parameter (ngroup_cat,npar_regr,npc_catdes)
  INTEGER, ALLOCATABLE, PUBLIC :: clust_group(:)  !<group id of catchment after catchments are grouped based on their charactersistics (nsub)
  REAL, ALLOCATABLE, PUBLIC    :: catdes(:,:)     !<catchment descriptors for catchments to be used in parameter regionalization (nsub,npc_catdes)
!> \}
