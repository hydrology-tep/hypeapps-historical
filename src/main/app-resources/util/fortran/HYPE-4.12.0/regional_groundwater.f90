...
CONTAINS
  ...
  SUBROUTINE initiate_aquifer_model(n,ns,na)
    ...
    dcoeff = 0. !no delay
    DO ia = 1,na
      IF(aquifer(ia)%percdelay>0.)THEN
        dcoeff(ia) = EXP(-1./(aquifer(ia)%percdelay*timesteps_per_day*(1.+regpar(m_aqdelcorr,aquifer(ia)%parregion))))
      ENDIF
    ENDDO
  ...
  !classes. 
  !!Removes the flow from the soil
  !!
  ...
  SUBROUTINE calculate_regional_groundwater_flow(soilstate,outputflow,classload)
    ...
    REAL, INTENT(OUT)   :: outputflow(4,nsub)         !<Outflow from groundwater (mm*m2/timestep) (1=all,2-4=per soillayer)
        ...
        DO j = 1,nclass
          ...
          IF(a>0)THEN
            ...
            IF(soilpar(m_rcgrwst,isoil)>0)THEN
              rcgrwj = soilpar(m_rcgrwst,isoil)*(1.+regpar(m_aqpercorr,basin(i)%parregion))
            ELSE
  ...
  !>Reduce OC-concentration of flow and remove the water from the soil layer.
  !>
  !> \b Reference ModelDescription Chapter Deep processes (Regional groundwater flow
  !> and Aquifers - Deep percolation)
  !--------------------------------------------------------------------------
  ...
  !>Reduce OC-concentration of flow and remove the water from the river.
  !>
   ...
   USE MODVAR, ONLY : basin,      &
    ...
    IF(flow>0.)THEN
      ...
      IF(riverstate%water(itype,i)>0)THEN
        waterfrac = riverstate%water(itype,i)/rivervol
        CALL set_percolation_concentration(n,riverstate%conc(:,itype,i),cflow)
        !Reduce the OC-concentration of the flow?
        CALL remove_water(riverstate%water(itype,i),n,riverstate%conc(:,itype,i),waterfrac*flow,cflow,status)
        IF(status.NE.0) CALL error_remove_water(errstring(3),subid,i,itype)
        accsubst = accsubst + cflow * waterfrac*flow*1.E3
        grwflow(i) = grwflow(i) + waterfrac*flow*1.E3   !mm*m2
      ENDIF
      DO l = 1,ttstep(itype,i)
        IF(riverstate%qqueue(l,itype,i)>0)THEN
          waterfrac = riverstate%qqueue(l,itype,i)/rivervol
          CALL set_percolation_concentration(n,riverstate%cqueue(:,l,itype,i),cflow)
          CALL remove_water(riverstate%qqueue(l,itype,i),n,riverstate%cqueue(:,l,itype,i),waterfrac*flow,cflow,status)
      ...
      IF(ttpart(itype,i)>0)THEN
        l = ttstep(itype,i) + 1
        IF(riverstate%qqueue(l,itype,i)>0)THEN
          waterfrac = riverstate%qqueue(l,itype,i)/rivervol    !Note whole volume so that pool get correct concentration change
          CALL set_percolation_concentration(n,riverstate%cqueue(:,l,itype,i),cflow)
          CALL remove_water(riverstate%qqueue(l,itype,i),n,riverstate%cqueue(:,l,itype,i),waterfrac*flow,cflow,status)
          IF(status.NE.0) CALL error_remove_water(errstring(4),subid,i,itype)
          accsubst = accsubst + cflow * waterfrac*flow*1.E3
  ...
  !>Reduce OC-concentration of flow and remove the water from the floodplain.
  !>
  !> \b Reference ModelDescription Chapter Deep processes (Aquifers)
  !--------------------------------------------------------------------------
  ...
  !>\brief Calculate percolation flows.
  !>
  !>\Calculate percolation from soil layers and rivers
  !!
  ...
  SUBROUTINE calculate_aquifer_percolation(perc,cperc)
    ...
    !Argument declarations
    REAL, INTENT(OUT)   :: perc(naquifers)            !<water leaving soillayer-model (m3/timestep) 
    REAL, INTENT(OUT)   :: cperc(numsubstances,naquifers) !<concentration of water -"- 
  ...
  SUBROUTINE calculate_aquifer(aquiferstate,aqoutflow,remflow2,aquiferirrloss)
    ...
    USE HYPEVARIABLES, ONLY : m_denitaq
    ...
    DO ia=1,naquifers
      CALL soil_denitrification(aquifer(ia)%maxvol,genpar(m_denitaq),aquiferstate%water(ia),aquifer(ia)%temperature,aquiferstate%conc(:,ia),sink)
    ENDDO
  ...
  SUBROUTINE calculate_aquifer_outflow(ia,aquiferstate,aqoutflow,caqoutflow)
    ...
    outpar = aquifer(ia)%retrate*(1.+regpar(m_aqretcorr,aquifer(ia)%parregion))
  ...
  !
  !>\b Consequences Module regional_groundwater_module variables addflow and caddflow will be set
  !>
  !> \b Reference ModelDescription Chapter Deep processes (Aquifers)
  !------------------------------------------------------------------------------
  ...
  FUNCTION calculate_aquifer_waterlevel(ns,na,aquiferstate) RESULT(subbwl)
    ...
    INTEGER i,ia    !loop-index, i_subbasin,ia-aquifer 
