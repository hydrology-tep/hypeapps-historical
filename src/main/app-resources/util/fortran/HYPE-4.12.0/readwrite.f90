...
600 FORMAT(A32,I7,A100)
  ...
  !-------------------------------------------------------------------------------
  SUBROUTINE check_station(funit,infile,ncols,nskip,geostn,nobsstn,oindex,n_Result) 
    ...
    INTEGER, INTENT(IN)  :: geostn(ncols)   !<subbasin id in GeoData.txt
    INTEGER, INTENT(OUT) :: nobsstn         !<Number of columns in file
    INTEGER, INTENT(OUT) :: oindex(ncols)   !<Index to find correct station in matrix
    INTEGER, INTENT(OUT) :: n_Result        !<error number
    
    !Local variables
    CHARACTER(LEN=100) str
    CHARACTER(LEN=18000) line     !Enough?
    INTEGER i,j,k
    ...
    READ(funit,*) str,stn(1:nobsstn)       !read stations in file
    CLOSE(funit)
...
903 WRITE(6,*) 'ERROR: reading file '//TRIM(infile)
  ...
  !----------------------------------------------------------------------
  SUBROUTINE write_mathsep(iout,n,x,ndec,textin,sep)
    ...
    t = ADJUSTL(t)
    lt = LEN_TRIM(t)
    outtxt(1:lt+1) = t(1:lt)//sep

    lout = lt+1
    form_f = '(ES16.'//CHAR(ndec+48)//')'
    ...
    DO i = 1,n
      ...
      lout = lout+lt
      IF(i < n) THEN
        outtxt(lout+1:lout+1) = sep        !Also the last one necessary
  ...
  !----------------------------------------------------------------------
  SUBROUTINE write_sepsep(iout,n,x,ndec,textin,sep,abb)
