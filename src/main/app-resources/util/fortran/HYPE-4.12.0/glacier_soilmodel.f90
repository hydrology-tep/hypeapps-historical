...
CONTAINS
  ...
  SUBROUTINE soilmodel_3(i,j,isoil,iluse,subid,classarea,prec,cprec,temp, & 
       daylength,cevpcorr,rrcscorr,phoscorr,  &
...
!    CALL calculate_snowdepth(frozenstate%snow(j,i),oldsnow,genpar(m_sndens0),genpar(m_dsndens),frozenstate%snowage(j,i),frozenstate%snowdepth(j,i))
    ...
    CALL soil_np_processes(i,j,conductN,conductP,conductC,dayno,classarea,wpmm(:,j),fcmm(:,j),epmm(:,j),plantuptake,soilthick(:,j),genpar(m_fertdays),genpar(m_littdays),  &
  ...
  !> \b Reference ModelDescription Chapter Land routines (Glaciers)
  !---------------------------------------------------------
  SUBROUTINE initiate_glacier(nc,arr,frozenstate)
