...
SUBROUTINE DEMC_simulation(dir,writeall,frozenstate,soilstate,aquiferstate,riverstate, &
      ...
      USE WORLDVAR, ONLY : nacrit,maxperf,            &
                           ...
                           fileunit_demc_debug, &
                           filename_demc_debug, &
                           fileunit_demc_candidates, &
                           filename_demc_candidates
...
!Debug print out to identify problematic parameters
      IF(optim%DEMC_debug)THEN
        OPEN(fileunit_demc_candidates, FILE = TRIM(dir)//filename_demc_candidates, STATUS = 'replace')   ! Replace demc candidates log file
        !write header
        !igen, ipop, depop1, depop2, cand1, cand2, ..., candN
        CALL write_demc_candidates_heading(fileunit_demc_candidates,npar)
        OPEN(fileunit_demc_debug, FILE = TRIM(dir)//filename_demc_debug, STATUS = 'replace')   ! Replace demc debug log file
        WRITE(fileunit_demc_debug,*)'DEMC DEBUG LOG:'
        WRITE(fileunit_demc_debug,*)'---------------'
        WRITE(fileunit_demc_debug,*)'demc started ...'
      ENDIF
...
!Initialize simulation counter
    ...
    IF(optim%DEMC_debug)THEN
        WRITE(fileunit_demc_debug,*)'initial settings done ...'
    ENDIF
...
!DEBUG
        IF(optim%DEMC_debug)THEN
          WRITE(fileunit_demc_debug,*)'------------------------------------------------------'
          WRITE(fileunit_demc_debug,*)'starting new iteration: generation',igen,'population',jpop       
        ENDIF

!Generate initial parameter vector 
        CALL get_randompar(numoptimpar,parmin,parmax,npar,parprop)
!Write candidate parameters to demc-candidates.log
        IF(optim%DEMC_debug)THEN
          WRITE(fileunit_demc_debug,*)'  candidates parameters ready ...'
          CALL write_DEMC_candidates_values(fileunit_demc_candidates,isim,igen,jpop,0,0,npar,parprop)
          WRITE(fileunit_demc_debug,*)'  candidates parameters written to log ...'
          WRITE(fileunit_demc_debug,*)'  calling model ...'
        ENDIF
!Run model, calculate optimization criteria
        ...
        ENDIF
        IF(optim%DEMC_debug)THEN
            WRITE(fileunit_demc_debug,*)'  model ready ...'
        ENDIF
...
!Save results in Population matrices (similar to bookkeepresults...)
        ...
        IF(optim%DEMC_debug)THEN
            WRITE(fileunit_demc_debug,*)'  accept/reject ready ...'
        ENDIF
!Print results to allsim.txt
        IF(writeall)THEN
            ...
            IF(optim%DEMC_debug)THEN
              WRITE(fileunit_demc_debug,*)'  writing to allsims.txt ready ...'
            ENDIF
        ENDIF
!Start printing progress to calibrationlog
        CALL write_DEMC_calibrationLog(igen, jpop)
        IF(optim%DEMC_debug)THEN
          WRITE(fileunit_demc_debug,*)'  writing to calibration.log ready - end of iteration ...'
        ENDIF
     ENDDO
...
!Random mutation population R1 and R2; without replacement excluding j
            ...
            IF(optim%DEMC_debug)THEN
              WRITE(fileunit_demc_debug,*)'  mutation populations R1 and R2  - ready ...'
            ENDIF
!Proposal parameter vector, by mutation of R1, R2 and j
            ...
            IF(optim%DEMC_debug)THEN
              WRITE(fileunit_demc_debug,*)'  proposal generation  - ready ...'
            ENDIF
!Crossover
            CALL DEMC_crossover(parprop,jpop,crossover,npar)
            IF(optim%DEMC_debug)THEN
              WRITE(fileunit_demc_debug,*)'  crossover  - ready ...'
            ENDIF
!Control parameter limits
            CALL DEMC_controlparameters(parprop,parmin,parmax,npar)
            IF(optim%DEMC_debug)THEN
              WRITE(fileunit_demc_debug,*)'  control parameters  - ready ...'
            ENDIF
!Write candidate parameters to demc-candidates.log
            IF(optim%DEMC_debug)THEN
              CALL write_DEMC_candidates_values(fileunit_demc_candidates,isim,igen,jpop,R1,R2,npar,parprop)
              WRITE(fileunit_demc_debug,*)'  candidates parameters written to log ...'
              WRITE(fileunit_demc_debug,*)'  calling model ...'
            ENDIF
!Run model, calculate optimization criteria
...
!Some printout?
            IF(writeall)THEN
              ...
              IF(optim%DEMC_debug)THEN
                WRITE(fileunit_demc_debug,*)'  write to allsims.txt ready ...'
              ENDIF
            ENDIF
!Some printout?
             CALL write_DEMC_calibrationLog(igen, jpop)
              IF(optim%DEMC_debug)THEN
                WRITE(fileunit_demc_debug,*)'  write to calibration.log ready - end of iteration!'
              ENDIF
         ENDDO
     ENDDO
     IF(optim%DEMC_debug)THEN
       WRITE(fileunit_demc_debug,*)'  ------------------------------------------------------------'
       WRITE(fileunit_demc_debug,*)'  DEMC end of all iterations'
     ENDIF
!Calculate and write MEDIAN parameterset and results to row 1 in bestMC:s, and the final populations to row 2:npop+1
     ...
     IF(optim%DEMC_debug)THEN
       WRITE(fileunit_demc_debug,*)'  Median parameters calculated and used for final model simulation'
       WRITE(fileunit_demc_debug,*)'  End of DEMC debug output - thanks for listening!'
     ENDIF

!Finish up
     CLOSE(fileunit_MC)
     IF(optim%DEMC_debug)THEN
       CLOSE(fileunit_demc_candidates)
       CLOSE(fileunit_demc_debug)
     ENDIF
