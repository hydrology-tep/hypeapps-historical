!><a
!>href=http://flibs.sourceforge.net/libdate.html>http://flibs.sourceforge.net/libdate.html</a>
MODULE LibDate
!
...
!
! This parameter is used to define all real numbers as REAL(Float)
! instead of just REAL. Include this parameter somewhere in your own
! code and make it publicly available to this MODULE by the proper USE
! statement, or change all references to REAL(Float) to REAL or to
! your own type. Beware: all REAL(Float) constants in this module also
! have "_Float" appended to ensure the right type!
!
...
& Date2Julian    ,& ! Conversion routine DateType --> JulianDateType
& Julian2Date    ,& ! Conversion routine JulianDateType --> DateType
& Julian2Dhm     ,& ! Conversion routine JulianDateType --> DateType(0,0,Days,Hours,Minutes)
& format_date    ,& ! Format a date/time structure
& SubtractDates  ,& ! Subtract one date from another, result as DateType
& AddDates          ! Add timestep to date
...
!
! Compute time expressed as days, hours, minutes in DateTime format
! from JulianDateTime. Modification of Julian2Date to fit needs to 
! calculate simulation period length in the hydrological HYPE model
!
! Added 2011-12-01 by Karin Nyström, SMHI
! 
! karin.nystrom@smhi.se
! http://www.smhi.se/
! http://hype.sf.org/
!
  TYPE(JulianDateType), INTENT(IN) :: JulianDate
...
!
  ...
  Dum = 60._Float*24._Float*JulianDate%Tail
  IF (DebugLevel.GT.0) WRITE(*,40) Dum
...
!
      ...
      TYPE(JulianDateType) :: JulianDate,JulianStep
...
!
 ...
 10   FORMAT('AddDates : BaseDate = ',5(1X,I5))
      IF (DebugLevel.GT.0) WRITE(*,15) TimeStep
 15   FORMAT('AddDates : TimeStep = ',5(1X,I5))
      JulianDate = Date2Julian(BaseDate)
      IF (DebugLevel.GT.0) WRITE(*,20) JulianDate
 20   FORMAT('AddDates : JulianDate = ',2(1X,F20.10))
      JulianStep = Date2Julian(TimeStep)
...
! Modified 2013-11-21 by Charlotta Pers, SMHI
! 
! charlotta.pers@smhi.se
! http://www.smhi.se/
! http://hype.sf.org/
!
   TYPE( DateType ), INTENT( IN ) :: Date1,Date2
