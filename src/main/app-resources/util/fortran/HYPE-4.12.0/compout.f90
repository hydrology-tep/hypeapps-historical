...
!with HYPE. If not, see <http://www.gnu.org/licenses/>.
  ...
  SUBROUTINE calculate_criteria(crit0,basincrit,simcrit,crit1,thres1)
    ...
    REAL   :: locnsw(nsub)          !NSE adjusted for bias
    ...
    DO iac = 1, nacrit
      ...
      IF(calcsubass.OR.calcsimass)THEN
        ...
        CALL calculate_variance_based_criteria(iac,nsub,stdr,stdc,stderr,locrs,loccc,locnsw)
      ...
      IF(calcsimass)THEN
        ...
        CALL calculate_mean_criterion(nsub,locnsw,simcrit(i_mnw,iac))            !Aritmetic mean of NSE adjusted for bias
    ...
    DO i = 1, ncrit
      IF(.NOT.calvar(i)%cond)THEN
        ...
        IF(str=='MNW')THEN                      !NSE adjusted for bias, mean over all subbasins
          ...
          ELSE
            CALL calculate_variance_based_criteria(iac,nsub,stdr,stdc,stderr,locrs,loccc,locnsw)
            CALL calculate_mean_criterion(nsub,locnsw,critadd)  
          ENDIF
          crit0 = crit0 - critadd * weight
        ENDIF
      ENDIF
