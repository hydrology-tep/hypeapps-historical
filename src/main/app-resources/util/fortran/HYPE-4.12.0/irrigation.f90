...
CONTAINS
  ...
  !--------------------------------------------------------------------------
  SUBROUTINE calculate_irrigation(i,na,regirrpar,pirrspar,pirrgpar,q,lakew,riverv,aquiferv,accpw,irrnetloss, &
       gwabstr,ldabstr,lrabstr,rsabstr,cq,cirrsinkpar,   &
       irrsinkmass,irrcomppar,cilake,colake,criver,caquifer,soilstate, &
    ...
    REAL, INTENT(IN)    :: regirrpar         !<parameter scaling the water withdrawal from regional source (-)
    REAL, INTENT(IN)    :: pirrspar          !<parameter scaling the irrigation water abstractions for surface water(-)
    REAL, INTENT(IN)    :: pirrgpar          !<parameter scaling the irrigation water abstractions for groundwater(-)
    ...
    REAL, INTENT(IN)    :: cirrsinkpar       !<parameter: concentration reduction fraction in settlement tanks (at irrigation abstraction points)
    REAL, INTENT(OUT)   :: irrsinkmass(numsubstances) !<mass of substances in the irrigation sink in this subbasin (typically in kg)
    REAL, INTENT(IN)    :: irrcomppar        !<parameter regulating the degree of comensation allowed between local groundwater/surfacewater sources (1 = allow 100% compensation, 0=do not allow compensation, default)
