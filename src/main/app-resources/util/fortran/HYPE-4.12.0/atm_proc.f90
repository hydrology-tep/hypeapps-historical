...
MODULE ATMOSPHERIC_PROCESSES
  ...
  SUBROUTINE set_atmospheric_parameters_corrections()
    ...
    IF(conductregest)THEN
      IF(indx_par(n_tcalt).gt.0) THEN
        DO i = 1,nsub
          basintcalt(i) = 0.
          DO k=1,ndes_reg(clust_group(i),indx_par(n_tcalt))
            basintcalt(i) = basintcalt(i) + wght_catdes(clust_group(i),indx_par(n_tcalt),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_tcalt),k))
          ENDDO
        ENDDO
      ENDIF
    ENDIF
    ...
    basintempadd = 0.
    IF(genpar(m_tcobselev)/=0.AND. ALLOCATED(forcing)) basintempadd = basintempadd - genpar(m_tcobselev)*(basin%elev-forcing%tobselev)*0.01    !adjust for Tobs elevation
    temppar1 = genpar(m_tcelevadd)
    DO i = 1,nsub
      IF(basin(i)%parregion>0)THEN
        temppar2 = regpar(m_tcadd,basin(i)%parregion)
      ELSE
        temppar2  = 0.
      ENDIF
      ...
      IF(conductregest)THEN
        IF(indx_par(n_tcea).gt.0) THEN
          temppar1=0.0
          DO k=1,ndes_reg(clust_group(i),indx_par(n_tcea))
            temppar1 = temppar1 + wght_catdes(clust_group(i),indx_par(n_tcea),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_tcea),k))
          ENDDO
        ENDIF
        IF(indx_par(n_tcorr).gt.0) THEN
          temppar2=0.0
          DO k=1,ndes_reg(clust_group(i),indx_par(n_tcorr))
            temppar2 = temppar2 + wght_catdes(clust_group(i),indx_par(n_tcorr),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_tcorr),k))
          ENDDO
        ENDIF
      ENDIF
    ...
    DO i = 1,nsub
      DO j= 1,nclass
        !Replace parameter values with regional parameter estimates
        IF(conductregest)THEN
          IF(indx_par(n_pcet).gt.0) THEN
            pcelevth=0.0
            DO k=1,ndes_reg(clust_group(i),indx_par(n_pcet))
              pcelevth = pcelevth + wght_catdes(clust_group(i),indx_par(n_pcet),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_pcet),k))
            ENDDO
          ENDIF
          IF(indx_par(n_pcea).gt.0) THEN
            pcelevadd=0.0
            DO k=1,ndes_reg(clust_group(i),indx_par(n_pcea))
              pcelevadd = pcelevadd + wght_catdes(clust_group(i),indx_par(n_pcea),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_pcea),k))
            ENDDO
          ENDIF
          IF(indx_par(n_pcem).gt.0) THEN
            pcelevmax=0.0
            DO k=1,ndes_reg(clust_group(i),indx_par(n_pcem))
              pcelevmax = pcelevmax + wght_catdes(clust_group(i),indx_par(n_pcem),k)*catdes(i,idx_catdes(clust_group(i),indx_par(n_pcem),k))
            ENDDO
          ENDIF
        ENDIF
    ...
    !>Replace parameter values with regional parameter estimates
    IF(conductregest)THEN
      IF(indx_par(n_pcur).gt.0) THEN
        basinpcurain = 0.
        DO isb = 1,nsub
          DO k=1,ndes_reg(clust_group(isb),indx_par(n_pcur))
            basinpcurain(isb) = basinpcurain(isb) + wght_catdes(clust_group(isb),indx_par(n_pcur),k)*catdes(isb,idx_catdes(clust_group(isb),indx_par(n_pcur),k))
          ENDDO
        ENDDO
      ENDIF
      IF(indx_par(n_pcus).gt.0) THEN
        basinpcusnow = 0.
        DO isb = 1,nsub
          DO k=1,ndes_reg(clust_group(isb),indx_par(n_pcus))
            basinpcusnow(isb) = basinpcusnow(isb) + wght_catdes(clust_group(isb),indx_par(n_pcus),k)*catdes(isb,idx_catdes(clust_group(isb),indx_par(n_pcus),k))
          ENDDO
        ENDDO
      ENDIF
    ENDIF
  ...
  SUBROUTINE calculate_class_atmospheric_forcing(i,j,radext,  &
    ...
    IF(ALLOCATED(tmini))THEN
      tmin = tmini(i)-basintcalt(i)*classbasin(i,j)%deltah*0.01 ! Adds SLC-hight dependent correction for minimum temperature
      tmax = tmaxi(i)-basintcalt(i)*classbasin(i,j)%deltah*0.01 ! Adds SLC-hight dependent correction for maximum temperature
    ENDIF
  ...
  SUBROUTINE calculate_subbasin_precipitation(n,temppobs,precarr,pcorricep)
    ...
    !DG 20141113 undercatch correction of precipitation, dependent on snowfall fraction, applied before general and regional corrections:
      ...
      IF(basin(i)%parregion>0)THEN
        preccorr = 1. + regpar(m_preccorr,basin(i)%parregion)   !Regional correction of precipitation
      ELSE
        preccorr  = 1.
      ENDIF
  ...
  SUBROUTINE calculate_rain_snow_from_precipitation(iluse,prec,temp,snowfall,rainfall,sffrac)
    ...
    USE MODVAR, ONLY : genpar,landpar,modeloption,p_snowfall
  ...
  SUBROUTINE calculate_extraterrestrial_radiation(n,jday,radext)
    ...
    USE MODVAR, ONLY : basin, pi, solar
    ...
    !Variables from Modvar:
    ! solar = 0.0820, solar constant [MJ/m2/min]
    ! pi = 3.1415927
    ! basin (BASINTYPE) with latitude info for each basin
    ...
    DO i = 1,n     
      ...
      ELSE
!        omega = acos(omega)    !Sunset hour angle,(FAO, ekv 25)
  ...
  SUBROUTINE calculate_vapour_pressures(tmean,tmin,tmax,rhmean,rhmin,rhmax,swrad,radext,actvap,satvap)
    ...
    !Initialize output variabeles as missing
